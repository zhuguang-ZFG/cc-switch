...
4. ~~Prompt-cache ratio tables mainly contain older Claude model names~~ **Done (2026-07-29):** `CacheRatio` gained 26 active model ids (claude bare aliases, gpt-5.5/5.6, grok family, glm/deepseek/minimax/mimo/sensenova); `CreateCacheRatio` gained 14. Existing keys untouched. Ratios follow the table's own conventions (cache read 0.1 for modern flagships / 0.25 for deepseek-class; create 1.25). Pre-change option snapshot: `tmp/naopts-cache-disk-before.json` (local only).
