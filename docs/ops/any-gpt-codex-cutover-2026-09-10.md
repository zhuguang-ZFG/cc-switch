...
- **codex CLI 环境事件与修复（当晚 19:47–20:15）**：19:47 起 codex.CMD 报「系统找不到指定的路径」，根因是 **codex 自升级器**（0.153.4→0.154.0）的安装窗口（文件 mtime 19:51；期间 shim/平台包路径瞬时残缺）。诊断期间 `npm i -g @openai/codex@0.15x.0` 两次均**未放置**别名列平台包 `@openai/codex-win32-x64`（npm alias-optionalDependencies 缺陷，实测复现）；临时以 `npm pack` 手动解压 0.153.4 平台包到平铺兄弟位支撑验证。20:12 自升级器再次完成升级，树自洽为 0.154.0（主包+嵌套平台包 `codex/node_modules/@openai/codex-win32-x64`，npm 对别名平台包采用嵌套布局），手动副本已撤。验证：node 直跑 / python subprocess / cmd `/d /s /c` 三路 `--version` 全通；git-bash 直调 .CMD 报路径错误属 MSYS 执行层怪癖（20:09 后复现），不影响用户原生终端。20:15 端到端复测：0.154.0 下 `codex exec` 经默认链完整跑通（74s，模型回复正常打印，codex 计 27,141 tok）；注意 ch126 的 NewAPI 记账行在成功窗口也标 `stream: error`（usage 尾帧上游丢失，不影响内容流）。自升级器会再次改树——升级窗口期 shim 可能瞬时残缺，属瞬态，等它装完即自愈。
