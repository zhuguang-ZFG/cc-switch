...
| 严格故障序 | 见 `docs/ops/zg-claude-routing.md`「Strict failover order」：GPT `#21→#124→#123→#131`（`#21`=DC公益 8317，`gpt-5.6-terra`/`luna`/`5.5` e2e 已验证 2026-07-27；`#131`=freemodel-work，FreeModel Token Plan，`gpt-5.6-sol`/`luna`/`terra`，**必须浏览器 UA**（header_override 已配，裸 UA 被 WAF 挂起），key 仅 work.freemodel.dev 池有效、cc.freemodel.dev 报 Insufficient balance，e2e 已验证 2026-07-27）；Haiku `#122`(40)→`#63`kimi(-20)（vyce `#125` 已 **status=2 整车停车**——502/524 硬挂，先毒化 sonnet 后毒化 haiku，恢复后 status=1 + sonnet models 加回即可）；Fable `#127`(50)→`#63`(-20)（#63 的 haiku/fable abilities 2026-07-27 深夜打开做最后兜底，mapping 本就存在）；GPT 实况梯：terra `#21`(60)→`#131`(45)（**#124 已 status=3 自动封禁**、#123 无 terra 能力，文档旧梯失效）、**gpt-5.5 仅剩 #21 独苗**、sol 仅 #131；Opus 主池实况 `#10`(w20)/`#20`(w40)/`#60`(w30)+**#11 k40-a5f6 仍活 w1**（文档旧称 status=2，实际在跑约 1% 流量）；**Sonnet `#132`freemodel-work GPT(ch_pri-19)→`#63`kimi(-20)→`#133`(-21)**（2026-07-27 晚改版：vyceai #125 502 硬挂且慢失败挂起 25s~6.5min，挡死 Sonnet 链致 Claude Code auto 分类器持续「temporarily unavailable」；sonnet models 已从 `#125.models` 剥离——**注意此 fork 按 `channels.models` 路由，`abilities.enabled=0` 不足以摘渠**；`#125` 现仅 haiku；vyceai 恢复后把 sonnet models 加回即可自动回主渠。同深夜 `#132` 从 -32 提至 -19——work 池实测 2-3s 快稳，升 Claude 故障第一梯队，注意高频率分类器流量会先吃 freemodel 免费额度）；**Claude→GPT 兜底**：`#129 gpt-terra-claude-fallback`（克隆 #21，**ch_pri=-30**，Opus→`gpt-5.6-terra`、Sonnet→`gpt-5.5`）→ `#130 gpt-123458-claude-fallback`（克隆 #124，**ch_pri=-31**，同映射；2026-07-27 晚新增——当日 #21/#129 上游 8317 auth 池耗尽致 Opus 兜底 503，双独立上游避免单点；e2e 已验证 opus 200 经兜底链）→ `#132 freemodel-claude-fallback`（克隆 #131，**ch_pri=-19**（2026-07-27 深夜从 -32 提权：work 池实测 2-3s 快稳，升 Claude 故障第一梯队，在 #63/#133 之上），Opus→`gpt-5.6-sol`、Sonnet→`gpt-5.6-luna`，FreeModel 免费池，浏览器 UA；强制提 pri e2e 已验证 opus 200/use_channel=[132]）；**Opus/Sonnet 二档真 Claude** `#133 freemodel-cc-claude`（克隆 #125 type=14，**ch_pri=-21/w5**（2026-07-27 晚从 15 降——免费池慢至 39s+/SSL 间歇 reset，在 Sonnet 链挡在 #63 前会毒化分类器；现定位=#63 之下、GPT 兜底 -30 系之上），`cc.freemodel.dev` 池，key `fe_oa_7151…` work+cc 双池可用，旧 `fe_oa_2617…` 仅 work 池；**伪装必须 `claude-cli` UA**（数据中心 IP + 浏览器 UA 被 403「only official Claude Code CLI」）+ **VPS 代理 `127.0.0.1:7890`**（VPS 直连 503）；手动权重、不进 dx 自动集；强制提 pri e2e 已验证 opus 200/use_channel=[133]）；本机 FQ ZG→AR2 |
