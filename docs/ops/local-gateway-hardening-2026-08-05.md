...
- 模板污染修复：捕获模板中的 `input` 只代表捕获时会话，曾携带 `sandbox_mode=read-only`、`approval policy=never` 和固定 cwd，导致 OMP fallback 到 Sol 后误报当前会话只读。relay 现在丢弃全部捕获 `input`，只重建调用方对话；调用方未提供 tools 时也删除捕获工具表。顶层中性 Codex 指令、请求指纹和调用方 system/developer 指令继续保留。
- 模板修复验证：live relay 重启后，直连 Sol 返回 `RUNTIME_CONTEXT_CLEAN`；强制 function call 返回 `runtime_permission_probe({"status":"writable_context_clean"})` 与 `finish_reason=tool_calls`。OMP 全工具探针期间上游发生两次 `SSL: UNEXPECTED_EOF_WHILE_READING`，NewAPI 短暂 auto-ban ch73；Guardian 随后恢复到 `status=1`、恢复队列 0。该 TLS 瞬态失败不计作 OMP 工具链通过，也未据此增加无界重试。
