...
CC Switch 管理的 CLI 工具（Claude Code、Codex、Grok Build、OpenCode、OpenClaw、Reasonix、Pi）需要 Node.js 环境。Kimi Code 不通过 npm 分发，可在 CC Switch 内直接安装并自更新。
...
# 国内用户如下载慢，使用镜像源
npm install -g @xai-official/grok --registry=https://registry.npmmirror.com
```

#### Kimi Code

Kimi Code 不提供 npm 包，请在 CC Switch 的「设置 → 关于」中使用内置的安装 / 升级功能，安装后也通过该入口自更新。
...
# 国内用户如下载慢，使用镜像源
npm install -g reasonix@latest --registry=https://registry.npmmirror.com
```
