...
面板分为两个 Tab：
- **应用专属供应商**：仅用于当前选中的应用（Claude Code / Claude Desktop / Codex / Grok Build / OpenCode / OpenClaw / Kimi Code / Reasonix / Pi）
- **统一供应商**：跨应用共享的配置
...
| 预设名称 | 说明 |
|----------|------|
| xAI 官方 | 使用 xAI 官方端点与 API Key |
| 自定义 | 手动配置 base_url、api_key 与模型 |
...
此功能覆盖 **Claude Code / Claude Desktop / Codex / Grok Build / OpenCode / OpenClaw / Kimi Code / Reasonix / Pi** 中带模型字段的供应商表单，适用于支持 `/v1/models` 端点的供应商。Codex OAuth 类供应商会按需从 ChatGPT Codex 后端获取实时模型列表。
...
Grok Build 使用单一 TOML 配置文件（`~/.grok/config.toml`），切换供应商时整文件写入：
...
| `base_url` | 是 | API 端点地址 |
| `api_key` | 是 | API 密钥 |
| `default_model` | 是 | 默认使用的模型 |
| `models` | 否 | 可用模型列表 |

### Kimi Code 配置格式

Kimi Code 使用 TOML 配置（`~/.kimi-code/config.toml`），CC Switch 以增量方式写入 `[providers.<name>]` 与 `[models.<alias>]`，并设置顶层 `default_model`：
...
> 💡 Kimi Code 的配置目录默认为 `~/.kimi-code/`，可在设置中修改（对应设置项 `kimi_config_dir`），也可通过环境变量 `KIMI_CODE_HOME` 覆盖。

### Reasonix 配置格式

Reasonix 使用 TOML 配置（POSIX 为 `~/.reasonix/config.toml`，Windows 为 `%APPDATA%\reasonix`），CC Switch 以增量方式追加 `[[providers]]` 条目并设置 `default_model`；API Key 写入同目录的 `.env`，由配置中的 `api_key_env` 引用：
...
> 💡 Reasonix 的配置目录可在设置中修改，也可通过环境变量 `REASONIX_HOME` 覆盖。

### Pi 配置格式

Pi 使用 JSON 配置（`~/.pi/agent/`）：供应商与模型写入 `models.json`（camelCase 的 `baseUrl` / `apiKey`），凭据存于同目录的 `auth.json`，应用级设置存于 `settings.json`：
...
> 💡 Pi 的配置目录默认为 `~/.pi/agent/`，可在设置中修改（对应设置项 `piConfigDir`），也可通过环境变量 `PI_AGENT_HOME` 覆盖。

## 统一供应商

统一供应商可以跨多个应用共享配置，适用于支持多种 API 格式的中转服务。
