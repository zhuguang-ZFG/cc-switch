...
面板分为两个 Tab：
- **应用专属供应商**：仅用于当前选中的应用（Claude Code / Claude Desktop / Codex / Grok Build / OpenCode / OpenClaw / Kimi Code / Reasonix）
- **统一供应商**：跨应用共享的配置
...
此功能覆盖 **Claude Code / Claude Desktop / Codex / Grok Build / OpenCode / OpenClaw / Kimi Code / Reasonix** 中带模型字段的供应商表单，适用于支持 `/v1/models` 端点的供应商。Codex OAuth 类供应商会按需从 ChatGPT Codex 后端获取实时模型列表。
