...
### Windows：界面显示「localhost 拒绝连接 / ERR_CONNECTION_REFUSED」

**常见原因**：用 `cargo build --release` 替换了 `~/.local/bin/cc-switch.exe`，未打包前端资源。WebView 会去连开发态 `http://localhost:3000`，因而拒绝连接；代理口 `:15721` 有时仍可健康。

**解决方法**：
1. 恢复带完整前端的安装包 / 先前可用的备份二进制（体积通常更大，约 32MB+）
2. 正式发布请使用 `pnpm tauri build`，不要只换 `cargo` 产物
3. 可选：清理 `%USERPROFILE%\.cc-switch\EBWebView` 下的 Cache 后重启

### Windows：启动提示「数据库版本过新」（例如库 v18 / 应用仅支持 v17）

**原因**：曾运行过更新 schema 的构建（如开发版把库升到 v18），随后又换回了只支持旧 `SCHEMA_VERSION` 的 UI 包。应用会 **fail-closed**，拒绝打开以免旧代码改写新库。

**解决方法**：
1. **优先**：安装与库版本匹配（或更新）的正式构建
2. **仅当确认迁移无破坏性、且你清楚风险时**（例如 v18 只做了 `created_at` TEXT→INTEGER 清洗、无新列）：可备份 `~\.cc-switch\cc-switch.db` 后执行 `PRAGMA user_version = <应用支持版本>;`，再启动。误降级可能造成数据损坏，勿对未知迁移使用
...
**解决方法**：
- Claude Code：支持热重载，通常即时生效；如未生效可关闭并重新打开终端，或重启 IDE
- Codex / Grok Build：关闭并重新打开终端
- Kimi Code：保存后立即生效，无需重启
...
**操作步骤**：
1. 选择对应应用的「官方登录」预设（如 Claude / Codex）
2. 点击「启用」
3. 重启对应的 CLI 工具
4. 按照 CLI 工具的登录流程操作
...
只有**官方订阅类**（Claude / Codex / Gemini / Kimi Code 官方登录、GitHub Copilot、Codex OAuth 反向代理）会在启用供应商后自动显示配额。**其他所有供应商**（包括 Token Plan 和第三方余额查询）都需要手动到供应商卡片的「用量查询」面板中打开开关并选择内置模板，因为同一个请求地址可能同时有"套餐"和"余额"两种查询模式，需要你自行选择。详见 [2.5 用量查询 → 手动启用](../2-providers/2.5-usage-query.md#手动启用内置模板--自定义脚本)。
