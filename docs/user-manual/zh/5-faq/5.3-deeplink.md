...
| `app` | 是 | 应用类型：`claude` / `codex` / `grokbuild` / `opencode` / `openclaw` / `kimicode`（别名 `kimi-code` / `kimi` / `hermes`）/ `reasonix`（别名 `reasonix-cli`）/ `pi` |
| `name` | 是 | 名称 |
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `apps` | 是 | 应用列表（逗号分隔，如 `claude,codex,grokbuild,opencode`；另支持别名 `grok`，等同 `grokbuild`） |
...
> ⚠️ Skill 导入仅支持 Claude 应用，无需指定 `app` 参数。
