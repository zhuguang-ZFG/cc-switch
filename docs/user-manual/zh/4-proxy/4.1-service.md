...
```toml
base_url = "http://127.0.0.1:15721/kimicode/v1"
```

**Reasonix**（`~/.reasonix/config.toml`）：
```toml
base_url = "http://127.0.0.1:15721/reasonix/v1"
```
...
> 💡 应用接管仅支持 Claude、Codex、Grok Build、Kimi Code、Reasonix、Pi 这 6 个应用；OpenCode / OpenClaw 不支持代理功能。
...
|------|------|
| 时间 | 请求时间 |
| 应用 | Claude / Codex / Grok Build / Kimi Code / Reasonix / Pi |
| 供应商 | 使用的供应商 |
| 模型 | 请求的模型 |
| Token | 输入/输出 token 数 |
| 延迟 | 请求耗时 |
