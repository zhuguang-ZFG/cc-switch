...
| Kimi Code | 同步到 Kimi Code | `~/.kimi-code/mcp.json` 的 `mcpServers` |
| Reasonix | 同步到 Reasonix | `~/.reasonix/config.toml` 的 `[[plugins]]` |

> ⚠️ **注意**：OpenClaw、Claude Desktop 和 Pi 暂不支持 CC Switch MCP 同步。MCP 功能支持 Claude、Codex、Grok Build、OpenCode、Kimi Code 和 Reasonix。

### 开关实现机制

当开启某个应用的开关时，CC Switch 会：

1. **更新数据库**：将服务器对应应用的启用状态设为 `true`
...
- **Kimi Code**：需存在 `~/.kimi-code/` 目录
- **Reasonix**：需存在 `~/.reasonix/` 目录（Windows 为 `%APPDATA%\reasonix`）
...
1. 点击「导入」按钮
2. 选择要导入的应用（Claude / Codex / Grok Build / OpenCode / Kimi Code / Reasonix）
...
Kimi Code 的 MCP 配置在独立的 `mcp.json` 文件中：
...
Reasonix 的 MCP 服务器以 `[[plugins]]` 表数组形式写入 `config.toml`：
