...
| `app` | Yes | App type: `claude` / `codex` / `grokbuild` / `opencode` / `openclaw` / `kimicode` / `reasonix` / `pi` |
| `name` | Yes | Name |

**App value aliases**: `kimicode` also accepts `kimi-code` / `kimi` / `hermes`; `reasonix` accepts `reasonix-cli`. For `resource=mcp`, the alias `grok` maps to `grokbuild`. Skill import (`resource=skill`) is Claude-only.
...
| Parameter | Required | Description |
|-----------|----------|-------------|
| `apps` | Yes | App list (comma-separated, e.g., `claude,codex,grokbuild,opencode`) |
...
```
ccswitch://v1/import?resource=mcp&name=mcp-fetch&apps=claude,codex,grokbuild&config=%7B%22command%22%3A%22uvx%22%2C%22args%22%3A%5B%22mcp-server-fetch%22%5D%7D
```
