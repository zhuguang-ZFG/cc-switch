...
### Windows: UI shows “localhost refused connection / ERR_CONNECTION_REFUSED”

**Typical cause**: Replacing `~/.local/bin/cc-switch.exe` with a bare `cargo build --release` binary that has no embedded frontend. The WebView then tries the dev URL `http://localhost:3000` and fails; the local proxy on `:15721` may still respond.

**Fix**:
1. Restore a full installer build / known-good backup binary (usually larger, ~32MB+)
2. For release builds use `pnpm tauri build`, not cargo-only replacement
3. Optionally clear caches under `%USERPROFILE%\.cc-switch\EBWebView` and restart

### Windows: “Database version too new” (e.g. DB v18 / app supports v17)

**Cause**: A newer build raised SQLite `user_version`, then you rolled back to an older UI binary. The app **fail-closes** so an older build cannot mutate a newer schema.

**Fix**:
1. **Preferred**: install a build whose `SCHEMA_VERSION` ≥ the on-disk database
2. **Only if you verified the migration is non-destructive** (e.g. v18 only normalized TEXT `created_at` to INTEGER millis, no new columns): back up `~\.cc-switch\cc-switch.db`, then `PRAGMA user_version = <supported>;` and relaunch. Never downgrade unknown migrations
...
- Grok Build: Restart Grok Build to apply changes
- Kimi Code: Takes effect immediately after switching
