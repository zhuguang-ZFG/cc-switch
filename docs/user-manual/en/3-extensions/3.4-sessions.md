...
| Kimi Code | `~/.kimi-code/sessions/` (plus `session_index.jsonl`) |
| Reasonix | `<state_dir>/sessions/` and `<state_dir>/projects/*/sessions/` |
...
> **Note**: The Session Manager covers the eight session sources listed above — every supported app except Claude Desktop.
...
> **Note**: Codex / Grok Build / OpenCode / OpenClaw / Kimi Code / Reasonix / Pi session resume flows do not yet include the directory picker and still use the session's original project directory.
