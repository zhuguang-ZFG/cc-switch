...
| Kimi Code | Sync to Kimi Code | `~/.kimi-code/mcp.json`'s `mcpServers` |
| Reasonix | Sync to Reasonix | `~/.reasonix/config.toml`'s `[[plugins]]` |

> **Note**: OpenClaw, Claude Desktop, and Pi do not currently support CC Switch MCP sync. MCP functionality is supported for Claude, Codex, Grok Build, OpenCode, Kimi Code, and Reasonix.

### Toggle Implementation

When enabling an app's toggle, CC Switch will:

1. **Update database**: Set the server's `apps.claude/codex/grokbuild/opencode/kimicode/reasonix` status to `true`
...
- **Kimi Code**: Requires `~/.kimi-code/` directory to exist
- **Reasonix**: Requires the Reasonix configuration directory (`~/.reasonix/` by default) to exist
...
2. Select the app to import from (Claude/Codex/Grok Build/OpenCode/Kimi Code/Reasonix)
...
Kimi Code keeps MCP servers in a separate file, `~/.kimi-code/mcp.json`:
...
Reasonix stores MCP servers as an array-of-tables named `[[plugins]]` inside its main `config.toml`:
