...
Most CLI tools managed by CC Switch (Claude Code, Codex, Grok Build, OpenCode, OpenClaw, Reasonix, Pi) are installed via npm and require a Node.js environment.
...
Kimi Code is installed and updated directly from the CC Switch About tab (self-update; no npm package).
