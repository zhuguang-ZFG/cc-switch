...
Configurable apps: Claude, Claude Desktop, Codex, Grok Build, OpenCode, OpenClaw, Kimi Code, Reasonix, Pi.
...
| Kimi Code Directory | `~/.kimi-code/` | Kimi Code configuration directory (also overridable via the `KIMI_CODE_HOME` environment variable) |
| Reasonix Directory | `~/.reasonix/` (`%APPDATA%\reasonix` on Windows) | Reasonix configuration directory (also overridable via the `REASONIX_HOME` environment variable) |
| Pi Directory | `~/.pi/agent/` | Pi agent configuration directory (also overridable via the `PI_AGENT_HOME` environment variable) |
...
Configure failover queues and automatic switching strategies by app (Claude/Codex/Grok Build). See [4.3 Failover](../4-proxy/4.3-failover.md) for details.
...
| Kimi Code | Current version, latest version |
| Reasonix | Current version, latest version |
...
Click the "Copy" button to copy to clipboard. For Grok Build, Kimi Code, Reasonix, and Pi, use the install/update buttons in the About tab, which run the correct command for each tool automatically.
