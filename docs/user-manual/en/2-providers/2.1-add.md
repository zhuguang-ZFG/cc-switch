...
- **App-specific Provider**: Only for the currently selected app (Claude Code / Claude Desktop / Codex / Grok Build / OpenCode / OpenClaw / Kimi Code / Reasonix / Pi)
...
This feature is available in model-aware provider forms for **Claude Code / Claude Desktop / Codex / Grok Build / OpenCode / OpenClaw / Kimi Code / Reasonix / Pi**, and works for providers that support the `/v1/models` endpoint. Codex OAuth providers fetch live model lists from the ChatGPT Codex backend on demand.
...
### Grok Build Configuration Format

Grok Build stores its configuration in `~/.grok/config.toml` (TOML). When switching providers, CC Switch writes the whole file, including the endpoint, API key, default model, and model list:
...
| `base_url` | Yes | API endpoint URL |
| `api_key` | Yes | API key |
| `default_model` | Yes | Default model to use |
| `models` | No | Model-specific configuration table |

## Universal Provider

Universal providers can share configurations across Claude Code / Codex / Grok Build, suitable for proxy services that support multiple API formats.
...
3. Fill in the common configuration:
   ...
   - API Key
   - Endpoint URL
4. Check the apps to sync to (Claude Code / Codex / Grok Build)
5. Save
