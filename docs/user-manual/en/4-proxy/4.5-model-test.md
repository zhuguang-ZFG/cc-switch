...
Starting from v3.13.0, Stream Check coverage is extended to **all eight managed apps: Claude / Claude Desktop / Codex / Grok Build / OpenCode / OpenClaw / Kimi Code / Reasonix**, including all OpenClaw protocol variants (such as `openai-completions`). OpenCode is auto-detected via npm package mapping; OpenClaw supports custom `auth-header` detection and handles edge cases like Bedrock error messages and `baseURL` fallback. Providers using official endpoints are skipped.
...
| Grok Build  | Grok Build Model | System default | Recommend a lightweight Grok model                 |
...
| Kimi Code   | Kimi Code Model | System default |                                                     |
| Reasonix    | Reasonix Model | System default |                                                     |
