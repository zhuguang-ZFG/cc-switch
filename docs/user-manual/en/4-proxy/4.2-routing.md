...
| Kimi Code Routing | Route Kimi Code requests |
| Reasonix Routing | Route Reasonix requests |
| Pi Routing | Route Pi requests |

Multiple app routings can be enabled simultaneously.

> OpenCode and OpenClaw do not support the proxy feature, so they have no routing toggle.
...
**Grok Build configuration change** (`~/.grok/config.toml`):
...
**Kimi Code / Reasonix configuration change**:

The provider entry's `base_url` in `~/.kimi-code/config.toml` or `~/.reasonix/config.toml` is pointed at `http://127.0.0.1:15721/kimicode/v1` or `http://127.0.0.1:15721/reasonix/v1` respectively.

**Pi configuration change**:

The `cc-switch-proxy` provider entry's `baseUrl` in `~/.pi/agent/models.json` is pointed at `http://127.0.0.1:15721/pi/v1`.

### Request Forwarding

When the routing service receives a request:

1. Identifies the request source (Claude/Codex/Grok Build/Kimi Code/Reasonix/Pi)
