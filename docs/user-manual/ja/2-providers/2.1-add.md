...
パネルは 2 つのタブに分かれています：
- **アプリ専用プロバイダー**：現在選択中のアプリ（Claude / Claude Desktop / Codex / Grok Build / OpenCode / OpenClaw / Kimi Code / Reasonix / Pi）専用
...
Grok Build のプリセットは Codex のサードパーティプリセットカタログを共有しています（上記「Codex プリセット」の中継サービス一覧を参照）。「カスタム」を選択すると、すべてのパラメータを手動で設定できます。
...
この機能は **Claude / Claude Desktop / Codex / Grok Build / OpenCode / OpenClaw / Kimi Code / Reasonix / Pi** のうちモデル項目を持つプロバイダーフォームで利用でき、`/v1/models` エンドポイントをサポートするプロバイダーに対応します。Codex OAuth 系プロバイダーでは、必要に応じて ChatGPT Codex バックエンドからライブモデル一覧を取得します。
...
Grok Build は TOML 形式の設定ファイル（`~/.grok/config.toml`）を使用します：
...
| フィールド | 必須 | 説明 |
|------|------|------|
| `models.default` | はい | 使用するモデルプロファイル名（`[model.xxx]` と一致する必要あり） |
| `model` | はい | 上流プロバイダーへ送信する実際のモデル名 |
| `base_url` | はい | API エンドポイントアドレス |
| `name` | はい | プロバイダーの表示名 |
| `api_key` | はい* | API キー（`env_key` で環境変数参照にすることも可能） |
| `api_backend` | いいえ | API バックエンドタイプ（通常 `responses`） |
| `context_window` | いいえ | モデルのコンテキスト長 |

## 統一プロバイダー

統一プロバイダーは Claude / Codex / Kimi Code / Reasonix 間で設定を共有でき、複数の API 形式をサポートする中継サービスに適しています。
...
3. 共通設定を入力：
   ...
   - API Key
   - エンドポイントアドレス
4. 同期するアプリにチェック（Claude / Codex / Kimi Code / Reasonix）
5. 保存
