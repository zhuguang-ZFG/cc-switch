...
| `app` | はい | アプリタイプ：`claude` / `codex` / `grokbuild` / `opencode` / `openclaw` / `kimicode` / `reasonix` / `pi`（`kimicode` には `kimi-code` / `kimi` / `hermes`、`reasonix` には `reasonix-cli` のエイリアスも使用可能） |
| `name` | はい | 名前 |
...
| パラメータ | 必須 | 説明 |
|------|------|------|
| `apps` | はい | アプリリスト（カンマ区切り、例：`claude,codex,grokbuild,opencode`。MCP では `grok` も `grokbuild` のエイリアスとして使用可能） |
...
> **注意**：Skill のインポートは Claude のみ対応しています。
