...
| Kimi Code | Kimi Code に同期 | `~/.kimi-code/mcp.json` の `mcpServers` |
| Reasonix | Reasonix に同期 | `~/.reasonix/config.toml` の `[[plugins]]`（Windows は `%APPDATA%\reasonix\config.toml`） |

> **注意**：OpenClaw、Claude Desktop、Pi は現在 CC Switch MCP 同期に対応していません。MCP 機能は Claude、Codex、Grok Build、OpenCode、Kimi Code、Reasonix に対応しています。

### スイッチの動作

あるアプリのスイッチをオンにすると、CC Switch は以下を実行します：

1. **データベースの更新**：サーバーの `apps.claude/codex/grokbuild/opencode/kimicode/reasonix` のステータスを `true` に設定
...
- **Kimi Code**：`~/.kimi-code/` ディレクトリが存在する必要あり
- **Reasonix**：`~/.reasonix/` ディレクトリ（Windows は `%APPDATA%\reasonix`）が存在する必要あり
...
1. 「インポート」ボタンをクリック
2. インポートするアプリを選択（Claude/Codex/Grok Build/OpenCode/Kimi Code/Reasonix）
...
Reasonix では MCP サーバーをプラグインとして `[[plugins]]` 配列に書き込みます（Windows は `%APPDATA%\reasonix\config.toml`）：
