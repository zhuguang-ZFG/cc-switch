...
## 追加（02:35）：GPT 回主力层 + 近期错误分析（用户决策修正）

- **用户决策修正**：GPT 渠道（muyuan #140 gpt-5.5、welfare #142 gpt-5.6-sol）
  从 40 层提回 **45 层主力**，和真 claude(#10/#20)、kimi(#138) 同层竞速。
  理由：① 公益站不追求缓存，速度快+智力够即可；② muyuan 0.01 倍率是福利不用白不用；
  ③ GPT 渠道要进 Claude Code 主力；④ 不用 codex。
- **撤销 optimizer cache 惩罚**：cache 命中率仍记录在日志（可见），但不再影响权重计算。
  公平竞争：速度+错误率定权重，不因上游缓存能力差异歧视公益站。
- **近期错误分析（6h 窗口）**：
  - **#140 muyuan**：85 次 500 全是 sensitive_words_detected（WAF 拦截，v5.2 已归 content
    类权重 0 不压权）。failover 正常：140->138 或 140->10，最终都成功。
  - **#142 welfare**：48 次 503（Service temporarily unavailable，夜间 02:00-02:16 限流），
    +19 次 403 +11 次 500。02:20 后恢复 200。被 optimizer 压到 w1，靠 failover 兜。
  - **#10 baibei**：15 次 502 + 6 次 500 + 1 次 524 + 1 次 429——真 claude 渠道偶发不稳定，
    failover 到 #20，最终成功。
  - **#41/#123/#134**：少量 429 并发限制，正常。
- **failover 链健康**：重试链覆盖 10/20/138/140/141/142，每次首渠道失败都能在 2-3 跳
