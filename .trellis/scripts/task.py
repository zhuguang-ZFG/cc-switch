...
Usage:
    ...
    python task.py checkpoint <dir> [--scope X] [--completed X] [--pending X] [--verification CMD=RESULT] [--phase P]
    python task.py resume <dir>                             # Show OMP recovery checkpoint (read-only)
"""
...
from common.omp_recovery import (
    build_recovery,
    capture_git_head,
    parse_verification,
    read_recovery,
...
def cmd_checkpoint(args: argparse.Namespace) -> int:
    """Save an OMP recovery snapshot into the task's task.json.

    The snapshot replaces any previous one (never merged) and is written
    atomically via common.io.write_json. Only meta.omp_recovery changes;
    every other task.json field and unknown meta key is preserved.
    """
    repo_root = get_repo_root()
    ...
    if not task_json_path.is_file():
        print(colored(f"Error: task.json not found at {full_path}", Colors.RED), file=sys.stderr)
        return 1

    verification = []
    for raw in args.verification:
        try:
            verification.append(parse_verification(raw))
        except ValueError as e:
            print(colored(f"Error: {e}", Colors.RED), file=sys.stderr)
            return 1
    ...
    try:
        recovery = build_recovery(
            ...
            git_head=capture_git_head(repo_root),
        )
    except ValueError as e:
        print(colored(f"Error: {e}", Colors.RED), file=sys.stderr)
        return 1
    ...
    try:
        saved = save_recovery(task_json_path, recovery)
    ...
    print(colored(f"✓ Recovery checkpoint saved to {task_json_path}", Colors.GREEN))
    print(f"  phase: {recovery['phase'] or '(none)'}  "
          f"scope: {len(recovery['scope'])}  "
          f"completed: {len(recovery['completed'])}  "
          f"pending: {len(recovery['pending'])}  "
          f"verification: {len(recovery['verification'])}")
    return 0


def _format_recovery_list(label: str, items: tuple[str, ...]) -> None:
    """Print a labeled list section; prints (none) for an empty list."""
    print(f"{label}:")
    if not items:
        print("  (none)")
        return
...
def cmd_resume(args: argparse.Namespace) -> int:
    """Display the stored OMP recovery checkpoint (read-only).

    Never mutates code, task state, or git. Warns when the current git
    HEAD differs from the checkpointed one.
    """
    repo_root = get_repo_root()
    full_path = resolve_task_dir(args.dir, repo_root)
    ...
    try:
        recovery = read_recovery(task_json_path)
    except ValueError as e:
        print(colored(f"Error: malformed checkpoint in {task_json_path}: {e}", Colors.RED), file=sys.stderr)
        return 1

    if recovery is None:
        print(colored(f"Error: no OMP recovery checkpoint found in {task_json_path}", Colors.RED), file=sys.stderr)
        return 1

    current_head = capture_git_head(repo_root)

    print(colored("OMP Recovery Checkpoint", Colors.BLUE))
    print("=" * 24)
    print(f"Task: {task_json_path}")
    print(f"Phase: {recovery.phase or '(none)'}")
    print(f"Updated: {recovery.updated_at}")
    print()
    _format_recovery_list("Scope", recovery.scope)
    print()
    _format_recovery_list("Completed", recovery.completed)
    print()
    _format_recovery_list("Pending", recovery.pending)
    print()
    print("Verification:")
    if not recovery.verification:
        print("  (none)")
    else:
        for item in recovery.verification:
            print(f"  - {item.command} => {item.result}")
    print()
    print(f"Checkpoint HEAD: {recovery.git_head or '(unavailable)'}")
    print(f"Current HEAD:    {current_head or '(unavailable)'}")
    if recovery.git_head and current_head and recovery.git_head != current_head:
        print(colored(
            f"Warning: git HEAD differs from checkpoint ({recovery.git_head} -> {current_head})",
            Colors.YELLOW,
        ))
    elif not recovery.git_head or not current_head:
        print(colored(
            "Note: git HEAD comparison not possible (checkpoint or current HEAD unavailable)",
            Colors.DIM,
        ))
    return 0
...
Usage:
  ...
  python task.py checkpoint <dir> [--scope S] [--completed C] [--pending P] [--verification CMD=RESULT] [--phase P]
                                                    Save OMP recovery snapshot into task.json
  python task.py resume <dir>                       Show OMP recovery checkpoint (read-only)
...
def main() -> int:
    ...
    p_checkpoint = subparsers.add_parser(
        "checkpoint", help="Save OMP recovery snapshot into task.json"
    )
    p_checkpoint.add_argument("dir", help="Task directory")
    p_checkpoint.add_argument(
        "--scope", action="append", default=[], help="Scope item (repeatable)"
    )
    p_checkpoint.add_argument(
        "--completed", action="append", default=[], help="Completed item (repeatable)"
    )
    p_checkpoint.add_argument(
        "--pending", action="append", default=[], help="Pending item (repeatable)"
    )
    p_checkpoint.add_argument(
        "--verification",
        action="append",
        ...
        help="Verification as COMMAND=RESULT (repeatable)",
    )
    p_checkpoint.add_argument(
        "--phase", default="", help="Workflow phase (e.g. implement)"
    )

    # resume
    p_resume = subparsers.add_parser(
        "resume", help="Show OMP recovery checkpoint (read-only)"
    )
