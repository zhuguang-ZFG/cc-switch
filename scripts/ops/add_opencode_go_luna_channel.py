#!/usr/bin/env python3
"""Onboard GPT 5.6 Luna from the OpenCode Go plan (zen/go endpoint) as a
standalone channel.

User decision 2026-08-22: wants Luna usable. Investigated the freebuff
reverse-proxy path first — its hardcoded 2026-07 registry predates Luna and
upstream now requires per-model roots (base2-free-luna), so the proxy would
need code changes. Then found the Go plan's own /v1/models lists
`gpt-5.6-luna`: verified live 2026-08-22 with the existing Go key (200 in
2.7s). Zero new infra, no ToS gray area — Luna rides the same Go dollar
quota as ch101/ch104 (table: $15/month allowance for Luna).

Model name exposed as-is (`gpt-5.6-luna`), no mapping. ModelRatio=0:
subscription plan, zero marginal cost. Single-channel model so priority is
cosmetic (50).
...
CHANNEL_NAME = "opencode-go-luna"
BASE_URL = "https://opencode.ai/zen/go"
MODELS = "gpt-5.6-luna"
TEST_MODEL = "gpt-5.6-luna"
MODEL_MAPPING = ""  # 模型名原样透出，无需映射
PRIORITY = 50
WEIGHT = 5
...
def online_backup(db_path: Path) -> Path:
    ...
    destination = backup_dir / (
        f"new-api-before-go-luna-{time.strftime('%Y%m%d-%H%M%S')}.db"
    )
...
def main() -> int:
    ...
    # (pool members share the model name by design); note them
    for i in items:
