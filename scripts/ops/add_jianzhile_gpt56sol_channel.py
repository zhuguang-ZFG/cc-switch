...
gateway (https://jianzhile.vip) as a last-resort backup.
...
docs/ops/jianzhile-channel-2026-08-13.md) and was refused then; the new key
passed the upstream admission probe on 2026-08-17 (/v1/models 200,
chat/completions 200), so the channel is now admitted.
...
def main() -> int:
    ...
    if not key:
        print("FATAL: pass the jianzhile API key as argv[1] (kept out of the repo)")
        return 2
