#!/usr/bin/env python3
"""Onboard the local freebuff2api proxy's MiMo (mimo/mimo-v2.5) as a backup
provider for the mimo-v2.5-free pool.

User decision 2026-08-22: freebuff free tier admitted this account at
accessTier=limited (countryBlockReason=country_not_allowed, egress
countryCode=TW) — only mimo/mimo-v2.5 is servable, 6 one-hour sessions/day
(Pacific-day reset). Luna and deepseek-v4-flash were probed and are NOT
available on this tier (403 session_model_mismatch); the proxy's model
registry is whitelisted to mimo accordingly.

Verified 2026-08-22 (direct proxy probes against 127.0.0.1:8321):
- mimo/mimo-v2.5: WORKS, non-stream and stream. Reasoning model — small
  max_tokens is eaten by hidden reasoning (max_tokens=64 returned empty
  content with finish_reason=length); relay probe uses 1024.
- deepseek/deepseek-v4-flash: admission with x-freebuff-model still lands
  on mimo (server ignores the header for limited tier) -> excluded.
- openai/gpt-5.6-luna: 403 session_model_mismatch on limited tier ->
  excluded.

The mimo free pool previously had a single provider (ch96 opencode-zen-free,
...
fallback. model_mapping {"mimo-v2.5-free": "mimo/mimo-v2.5"} keeps the
pool-visible name unchanged. ModelRatio=0: free source, zero marginal cost.

Upstream dependency: the freebuff2api local proxy (127.0.0.1:8321) must be
running. It autostarts at logon via HKCU Run key "Freebuff2API" (wscript +
run.cmd wrapper; scheduled-task creation needs elevation, hence the Run key).
If the proxy is down this channel 502s and NewAPI failover kicks in.
...
The NewAPI channel key field cannot be empty; the local proxy does not
authenticate (API_KEYS empty), so a placeholder passed via argv is stored
(never printed). Re-running is verify-only and never touches an existing
channel's status or key.
"""
...
CHANNEL_NAME = "freebuff-mimo"
BASE_URL = "http://127.0.0.1:8321"  # type=1 自动拼 /v1，base 不带 /v1
MODELS = "mimo-v2.5-free"
TEST_MODEL = "mimo-v2.5-free"
MODEL_MAPPING = json.dumps({"mimo-v2.5-free": "mimo/mimo-v2.5"})
PRIORITY = 9
WEIGHT = 5
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("key", help="local placeholder key (proxy does not authenticate; never printed)")
    parser.add_argument(
        "--apply",
        action="store_true",
...
def relay_probe(smoke) -> None:
    ...
    status, body = smoke.http_json(
        ...
        body={
            ...
            "max_tokens": 1024,  # mimo 是推理模型，小 max_tokens 会被隐式推理吃光、content 为空
        },
...
def online_backup(db_path: Path) -> Path:
    ...
    destination = backup_dir / (
        f"new-api-before-freebuff-mimo-{time.strftime('%Y%m%d-%H%M%S')}.db"
    )
...
def main() -> int:
    ...
    # (mimo-v2.5-free aggregates by design); note them
    for i in items:
