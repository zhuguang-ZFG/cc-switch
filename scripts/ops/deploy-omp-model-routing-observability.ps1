...
if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "Source extension does not exist: $source" }
$destinationDirectory = Split-Path -Parent $destination
if (-not $destinationDirectory) { throw "Destination must include a parent directory: $destination" }
...
try {
    ...
    if ($sourceHash -ne (Get-FileHash -LiteralPath $stagingPath -Algorithm SHA256).Hash) { throw "Staged extension SHA-256 mismatch." }
    ...
    if ($sourceHash -ne $destinationHash) { throw "Deployed extension SHA-256 mismatch." }
    [ordered]@{
    ...
    [pscustomobject]@{ Destination = $destination; Sha256 = $destinationHash; BackupDirectory = $backupDirectory; PreviousDestinationExisted = $hadDestination; RestartPerformed = $false }
} catch {
    try {
        if ($hadDestination -and (Test-Path -LiteralPath $previousCopy -PathType Leaf)) {
            ...
            if (Test-Path -LiteralPath $destination -PathType Leaf) { [System.IO.File]::Replace($rollbackStagingPath, $destination, $null, $true) }
            else { [System.IO.File]::Move($rollbackStagingPath, $destination) }
        ...
        if (Test-Path -LiteralPath $rollbackStagingPath) { Remove-Item -LiteralPath $rollbackStagingPath -Force }
    }
    throw
}
