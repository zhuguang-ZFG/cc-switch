...
- bai 免费池纯化脚本：`scripts/ops/sync_bai_free_models.py`（ch111 `bai-free` 2026-09-04 纯化至哨兵验证真免费 2 模型=**mimo-v2.5,hy3**；摘 mimo-v2.5-pro(503 陈旧)、hy4-preview/minimax-m2.7(余额门槛型：余额跌破模型要求即全 400)、deepseek-v4-flash/deepseek-v4-flash-vision-exp(哨兵实测扣费 -9/-30 quota)；教训=**200 OK ≠ 免费**；deepseek-v4-flash 全局 0.5 比率保留（ch110/ch118 共用）；回加条件=充值+直连200+哨兵零消耗，runbook 见 `docs/ops/bai-free-model-sync-2026-09-04.md`）
