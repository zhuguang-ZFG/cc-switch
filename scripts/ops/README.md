...
- bai 免费池模型同步脚本：`scripts/ops/sync_bai_free_models.py`（ch111 `bai-free` 2026-09-04 对齐上游免费目录：摘 503 陈旧 mimo-v2.5-pro、摘余额门槛 400 的 hy4-preview（上游要求 ~5010 credit、账户 ~2600 不达标；回加条件=充值+新一轮直连 200）、增 minimax-m2.7（ModelRatio=0）；403 付费档与仅 429 模型一律不收，封闭式单模型 ch113/ch121/ch122 不动，runbook 见 `docs/ops/bai-free-model-sync-2026-09-04.md`）
