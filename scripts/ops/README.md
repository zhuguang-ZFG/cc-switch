...
- jianzhile sol 渠道：`scripts/ops/add_jianzhile_gpt56sol_channel.py` + `scripts/ops/fix_jianzhile_codex_channel.py`（ch91，双 key 轮询，priority 10/weight 5；真实 Codex 0.147 直连 200、经 NewAPI 曾 403，根因是默认转发丢失动态 `X-Codex-*`/session/request-id 头；现用 `header_override {"*":""}` 安全透传完整指纹，并以条件 `param_override` 保留真实 Codex 动态头。OMP 的 Chat Completions 请求在命中 ch91 时按 channel-local policy 转为 Responses；专用 OMP alias `jianzhile-codex-gpt-5.6-sol` 已实测 E2E 200。该上游只可靠支持流式 Responses，Guardian 对 ch91 使用显式 `endpoint_type=openai-response&stream=true` 探针并保留正常 auto-ban/recovery；runbook 见 `docs/ops/jianzhile-gpt56sol-channel-2026-08-17.md`）
