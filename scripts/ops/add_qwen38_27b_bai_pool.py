#!/usr/bin/env python3
"""Pool qwen3-8-27b: add B.AI (api.b.ai) as a third source, 1:1:1 with
ch88 (runinfra) and ch112 (yjs) (2026-08-28).

Background
----------
After the two-source ch88+ch112 pool (see ``add_qwen38_27b_pool.py`` and
``docs/ops/qwen38-27b-aggregation-2026-08-28.md``), a re-scan of the B.AI
free tier (ch111 ``bai-free``, ``https://api.b.ai``) on 2026-08-28 found
that B.AI's free catalog grew after ch111's whitelist was cut: its
``/v1/models`` now lists 44 ids, and a live completion probe of
``qwen3.8-27b`` with the ch111 key returned HTTP 200
(``model=Qwen/Qwen3.8-27B-FP8``, content 'pong') while premium ids
(claude-opus-5, kimi-k3) return 403 "deposit required" — so the free-tier
boundary moved and the whitelisted 5-model set is no longer the whole free
set. B.AI upstream id is the dotted form ``qwen3.8-27b`` (third relay
spelling of the same open-weight release; yjs uses ``qwen-3.8-27b``).

Design
------
Same dedicated single-model channel pattern as ch88/ch112: ch111's
channel-level priority=30 is its deepseek-v4-flash pool tier, and per-model
weight cannot be set without moving that tier (abilities rows mirror
channel fields, proven 2026-08-28), so extending ch111 is out.

1. POST new channel ``bai-qwen3-27b`` (坑1: ``{"mode":"single","channel":
   {...}}`` wrapper, no ``status`` field): base_url ``https://api.b.ai``,
   key copied from ch111, type=1, models ``qwen3-8-27b``, mapping
   ``qwen3-8-27b -> qwen3.8-27b``, group ``default``, priority=0, weight=1,
   auto_ban=1, test_model ``qwen3-8-27b``.
2. ch88/ch112 unchanged: all three single-model channels at priority=0,
   weight=1 -> 1:1:1 weighted mix.
3. No OMP change: model id ``qwen3-8-27b`` unchanged. The free tier's rate
   limits are unprobed at scale; auto_ban=1 routes around bursts (a 429'd
   B.AI source simply cools out of the mix until AutomaticEnable).

Safety: online DB snapshot first, API+DB readback verify (all three
abilities rows), 12-request gateway functional test with channel
distribution from the ``logs`` table, automatic channel DELETE +
...
CH_KEY_SOURCE = 111    # bai-free (key donor; NOT modified)
NEW_CHANNEL_NAME = "bai-qwen3-27b"
...
def verify(channels: list[dict], new_id: int) -> None:
    ...
    row_secondary = db_row(112, MODEL)
    ...
    if row_secondary != ("default", 1, 0, 1):
        raise RuntimeError(f"ch112 abilities row changed unexpectedly: {row_secondary}")
...
def main() -> int:
    ...
    print(f"plan: ch{CH_PRIMARY} unchanged (abilities {row_primary}; single-model channel, "
          f"1:1:1 weighted mix with ch112 at equal priority)")
    ...
    try:
        ...
        if new_id not in distribution:
            print("WARNING: new channel did not receive any of the 12 requests "
                  "(1:1:1 weighted makes this ~0.05%); observe logs before assuming a fault")
        print(f"OK: qwen3-8-27b now pooled ch{CH_PRIMARY}+ch112+ch{new_id} 1:1:1; "
