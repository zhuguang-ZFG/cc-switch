#!/usr/bin/env python3
"""Onboard OpenCode Go plan's MiMo-V2.5 (highest-allowance Go model) to NewAPI.

User decision 2026-08-21: among Go-plan models, MiMo-V2.5 has the largest
monthly allowance (official table: ~150,400 req/month typical, $60 included
quota tier; only Muse Contributor is higher but it trains on data and was
recalled upstream). Verified 2026-08-21 against https://opencode.ai/zen/go/v1
with the Go key: mimo-v2.5 chat/completions 200. The Go subscription itself
is alive — only the muse model died (RegionError), which is why this is a NEW
channel and not a revival of ch48 (permanently exclusion-locked).

Distinct from ch96's `mimo-v2.5-free`: the Go-plan `mimo-v2.5` is the
standard zero-retention tier (Zen privacy section only carves out the *Free*
suffixed models for training data collection), so it is acceptable as a
general fallback where the free pool is not.

Channel params: type=1, base_url=https://opencode.ai/zen/go (NewAPI appends
/v1/chat/completions), browser-UA header_override required (Cloudflare,
same as ch96), p10/w5, auto_ban=1. ModelRatio=0: the Go plan is a flat
subscription, so per-call marginal cost is zero and cost logs should say so.
...
def online_backup(db_path: Path) -> Path:
    ...
    destination = backup_dir / (
        f"new-api-before-opencode-go-mimo-{time.strftime('%Y%m%d-%H%M%S')}.db"
    )
...
def main() -> int:
    ...
    # model-name collision check: another channel already serving mimo-v2.5
    # would silently aggregate with us
    for i in items:
