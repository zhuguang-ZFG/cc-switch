#!/usr/bin/env python3
"""Add tokenrouter.com free model channel (2026-08-28).

Background
----------
The user provided a tokenrouter.com key whose token-scoped catalog lists
exactly one free model, ``qwen/qwen3.8-max-free``. Live probe (2026-08-28):
completion HTTP 200, ``model=qwen3.8-max-pd`` (reasoning model, 25
reasoning tokens), content 'pong', finish stop, 4.3s. A second token for
the same host lists ``moonshotai/kimi-k3-free`` but returns 503
"no available channel" on every attempt, so it is NOT registered (dead
target lesson: zai-glm-5-2).

Design
------
Same dedicated single-model channel pattern as ch88/ch112/ch113:
- channel ``tokenrouter-qwen3.8-max-free`` (ch114): base_url
  ``https://api.tokenrouter.com`` (no trailing /v1 — NewAPI type=1 appends
  it; the 2026-08-01 three-pit), key from the TR_KEY environment variable
  (never printed or written anywhere but the channel row), models
  ``qwen3.8-max-free``, mapping ``qwen3.8-max-free ->
  qwen/qwen3.8-max-free``, group ``default``, priority=0, weight=1,
  auto_ban=1, test_model ``qwen3.8-max-free``.
- Exposed model id ``qwen3.8-max-free`` is a NEW OMP model (free-variant
  naming precedent: ``muse-spark-1.2-contributor-free``); it does not join
  the paid ``qwen3.8-max`` pool (ch31, Aliyun Token Plan) — free relay
  rate-limit behavior must not perturb the paid pool.
- No ModelRatio entry: the new id bills at the default 1.0 against the
  local wallet (same accepted state as qwen3-8-27b since 2026-08-27).

Safety: online DB snapshot first, API+DB readback verify, 12-request
gateway functional test with channel attribution from the ``logs`` table,
automatic channel DELETE + abilities-row cleanup on failure. Dry-run by
default. The TR_KEY env var must be set for --apply runs.
"""
...
UPSTREAM_ID = "qwen/qwen3.8-max-free"
BASE_URL = "https://api.tokenrouter.com"
NEW_CHANNEL_NAME = "tokenrouter-qwen3.8-max-free"
...
def online_backup(db_path: Path) -> Path:
    ...
    destination = backup_dir / f"new-api-before-tokenrouter-{stamp}.db"
...
def main() -> int:
    ...
    if any(str(c.get("name")) == NEW_CHANNEL_NAME for c in channels):
        ...
        return 0
    print(f"plan: POST {NEW_CHANNEL_NAME!r} base_url={BASE_URL} key=<TR_KEY env> "
    ...
    if not key:
        print("abort: TR_KEY environment variable not set (required for --apply)")
        return 1
    ...
    try:
        ...
        print(f"OK: {MODEL} now served by ch{new_id} {NEW_CHANNEL_NAME}; "
