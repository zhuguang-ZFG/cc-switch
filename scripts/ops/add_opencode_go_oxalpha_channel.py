#!/usr/bin/env python3
"""Onboard OpenCode Go plan's Ox Alpha (ox-alpha-free) as the paid-quota
reliability floor for the x-preview-f-free pool.

User decision 2026-08-21: x-preview-f-free (Ox Alpha) is promoted to a main
workhorse model and must be stabilized. The existing pool is free-tier only:
ch96 Zen free (intermittent 503/EOF, community-reported ~14% unavailability),
ch102/ch103/ch100 all proxy the SAME OpenRouter account-wide free daily cap
(1000 req/day, verified identical limit_source=openrouter_free_tier_daily).
When that cap exhausts, the whole pool dies until the 08:00 +08 reset.

This channel adds an independent path: the Go plan's $60/month dollar quota
(verified 2026-08-21: ox-alpha-free live on https://opencode.ai/zen/go/v1 with
the existing Go key, 200 OK, reasoning_tokens present). Placed at priority 4 —
BELOW openrouter-ox-alpha p5 — so it only takes traffic when every free
upstream is down, capping Go-quota burn while guaranteeing the pool never
fully dies.

Channel params mirror add_opencode_go_mimo_channel.py (type=1,
base_url=https://opencode.ai/zen/go, browser-UA header_override, auto_ban=1),
plus model_mapping {"x-preview-f-free": "ox-alpha-free"} so the pool-visible
model name is unchanged. ModelRatio=0: flat subscription, zero marginal cost.
...
Key comes from argv and is never printed. Re-running is verify-only and
never touches an existing channel's status or key.
"""
...
CHANNEL_NAME = "opencode-go-oxalpha"
BASE_URL = "https://opencode.ai/zen/go"
MODELS = "x-preview-f-free"
TEST_MODEL = "x-preview-f-free"
MODEL_MAPPING = json.dumps({"x-preview-f-free": "ox-alpha-free"})
PRIORITY = 4
WEIGHT = 5
...
MODEL_RATIO = 0  # Go 套餐订阅制，单次调用边际成本为零
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("key", help="OpenCode Go API key (never printed)")
    parser.add_argument(
        "--apply",
        action="store_true",
...
def online_backup(db_path: Path) -> Path:
    ...
    destination = backup_dir / (
        f"new-api-before-opencode-go-oxalpha-{time.strftime('%Y%m%d-%H%M%S')}.db"
    )
...
def main() -> int:
    ...
    # (x-preview-f-free aggregates by design); note them for visibility
    for i in items:
        ...
        if existing is not None:
            ...
            if json.loads(original_ratio).get(TEST_MODEL) != MODEL_RATIO:
                put_option(smoke, headers, MODEL_RATIO_OPTION,
                           ensure_ratio(original_ratio))
                ratio_changed = True
                print(f"ModelRatio=0 set for {TEST_MODEL} (subscription plan)")
            else:
