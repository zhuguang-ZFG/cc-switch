...
if (-not $SourcePath) {
    $SourcePath = Join-Path $PSScriptRoot "omp-sota-escalation.js"
}
if (-not $DestinationPath) {
    if (-not $env:USERPROFILE) {
        throw "USERPROFILE is required when DestinationPath is omitted."
    }
    $DestinationPath = Join-Path $env:USERPROFILE ".omp\agent\extensions\omp-sota-escalation.js"
}
...
$backupDirectory = Join-Path $backupRootPath "omp-sota-escalation-$stamp-$PID"
