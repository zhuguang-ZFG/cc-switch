...
def _critical_chain_entries(text: str) -> dict[str, list[str]]:
    """从 config.yml 提取关键 role 的 fallback 项；保持依赖为标准库。"""
    result = {role: [] for role in CRITICAL_ROLES}
    ...
    for raw_line in text.splitlines():
        ...
        if indent == 4 and stripped.endswith(":"):
            role = stripped[:-1]
            active_role = role if role in result else None
            continue
        if active_role and indent == 6 and stripped.startswith("- "):
            result[active_role].append(stripped[2:].strip())
    return result
...
class OmpRouteGateTests(unittest.TestCase):
    ...
    def test_critical_chains_exclude_known_bad_agentrouter_claude(self):
        chains = _critical_chain_entries(CONFIG_FILE.read_text(encoding="utf-8"))
