...
const deployScript = join(here, "deploy-omp-global-compaction-model.ps1");
...
test("deploys atomically with absence marker, backup, and matching hashes", () => {
  const root = mkdtempSync(join(tmpdir(), "omp-compaction-deploy-"));
  try {
    ...
    const backupDirectories = readdirSync(backups).map((name) =>
      join(backups, name),
    );
    assert.equal(backupDirectories.length, 2);
    const replacementBackup = backupDirectories.find((path) => {
      try {
        return readFileSync(join(path, "previous.js"), "utf8") === first;
      } catch {
        return false;
      }
    });
