#!/usr/bin/env python3
"""Onboard api.608885.xyz's stealth/ox-alpha as the fourth upstream for Ox Alpha.

Verified 2026-08-21 direct against https://api.608885.xyz/v1:
- KEY FORM GOTCHA: the site hands out a base64-encoded key
  ("c2st..." = base64 of "sk-..."); the raw base64 string gets 401,
  the decoded sk- form works. This script auto-decodes a "c2st"-prefixed
  argv key.
- /v1/models lists 5 models (grok-4.5, grok-4.6, moonshotai/kimi-k3,
  stealth/ox-alpha, zai/glm-5.2); this channel only maps ox-alpha.
- chat/completions 200: non-stream 1.8s, stream 3.6s, no cold start seen
- usage.cost=0 / upstream_inference_cost=0 — free on this relay too, so
  ...
  truthful. Usage shape mirrors Zen "Console" fields (likely Zen-backed,
  but independent infra).

Aggregation: declares public name x-preview-f-free with model_mapping to
stealth/ox-alpha, joining ch96 (Zen p10), ch102 (168661 p7), ch100
(OpenRouter p5). Posture p6/w5: Zen > 168661 > 608885 > OpenRouter —
608885 is fast and free, so it sits above the paid-account OpenRouter
floor. auto_ban=1. Browser-UA header_override kept (probes ran with it;
site protection posture unverified without).
...
CHANNEL_NAME = "s608885-ox-alpha"
BASE_URL = "https://api.608885.xyz"  # NewAPI appends /v1/chat/completions
PUBLIC_MODEL = "x-preview-f-free"
UPSTREAM_MODEL = "stealth/ox-alpha"
MODELS = PUBLIC_MODEL
MODEL_MAPPING = json.dumps({PUBLIC_MODEL: UPSTREAM_MODEL})
PRIORITY = 6  # zen ch96 p10 > 168661 ch102 p7 > 608885 p6 > openrouter ch100 p5
WEIGHT = 5
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "key",
        help="608885 ox-alpha key (never printed); a base64 'c2st...' form "
        "is auto-decoded to the sk- form the API actually accepts",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
...
def management_probe(smoke, headers: dict[str, str], channel_id: int) -> str:
    ...
    status, body = smoke.http_json(
        ...
        headers=headers,
        timeout=100,  # generous: sibling upstream 168661 once cold-hung >90s
    )
...
def online_backup(db_path: Path) -> Path:
    ...
    destination = backup_dir / (
        f"new-api-before-608885-ox-alpha-{time.strftime('%Y%m%d-%H%M%S')}.db"
    )
...
def main() -> int:
    ...
    if key.startswith("c2st"):  # base64("sk-...") as handed out by the site
        try:
            decoded = base64.b64decode(key).decode()
        except Exception as error:
            raise RuntimeError(f"key looks base64 but does not decode: {error}")
        if not decoded.startswith("sk-"):
            raise RuntimeError("decoded key does not start with sk-; refusing")
        key = decoded
