...
- **OpenCode Go Muse contributor guarded cutover (2026-08-19)**: moved the OMP task/tiny route from Luna to the lower-cost `muse-spark-1.2-contributor` after the workspace training opt-in. The transactional stage/finalize/rollback tool uses a model-level Responses API override, explicit contributor data-policy consent, and a fresh revision-r5 no-fallback Canary gate. Staged and post-final native-read proofs passed in 14.5s and 8.9s, with independent NewAPI logs attributing the requests to ch48. Luna was removed from ch48, `models.yml`, roles, and fallback references; production smoke now probes Muse over `/v1/responses`. See `docs/ops/opencode-go-muse-cutover-2026-08-19.md`.
...
- **agentrouter Opus 修复 + 日志排查治理（2026-08-15）**: ① agentrouter-proxy `_is_retryable` 补 429 与 402/403 额度关键词冷却换 key（认证 403 快速失败、500 敏感词不重试防计费放大），单元 9/9 + supervisor 自愈 + 直连/OMP 双 opus 端到端全过；② OMP bigctx 链首加 `longcat/LongCat-2.0`（修无 ≥380k 后备缺口）、agentrouter 并发 4→2；slow 链加 agentrouter 被 route gate 拒绝（敏感词误杀约束，已回退勿再试）；③ ai.168661 key 失效（ch78/79 401）OMP 摘除 deepseek-v4-flash-0731/hy3；④ guardian 心跳 WinError 5 重试修复（唯一 tmp 名 + 退避 ×3），141/141 测试含新增重试用例；⑤ 日志排查：主链路全绿，opus-5 ch76 单点待新源，ch82(7758) 上游仍 503 保持禁用，ch31 周配额耗尽待重置；⑥ anyrouter 窗口调查：上游 Claude 池 429/sol 容量满/gemini 无路径，本次窗口无成功路径，签到脚本只续额度不解 429。详见 `docs/ops/agentrouter-opus-retry-fix-2026-08-15.md`。
- **闲置模型上岗（2026-08-15 晚）**: `sensenova-6.7-flash-lite` 补 `reasoning: true`（实测返回 reasoning 字段，缺标记时 OMP 解析为空）并入三条链尾（tiny/commit/smol，免费池 429 吸收层）；`mercury-2`→smol 链尾、`agnes-2.5-pro-alpha`→vision 链尾、`intern-s2-preview`→plan 链尾（canary 全 200，全链尾零日常流量）。`gpt-5.5`（全 carrier abilities 禁用）与 `zai-glm-5-2`（muyuan 自身池断供）保持条目待自愈。时延注意：sensenova 思考会吃光小 max_tokens（≥4096 正常，~3s）；`enable_thinking=false`/`reasoning_effort=low` 上游均忽略。route gate 33/33。详见 `docs/ops/agentrouter-opus-retry-fix-2026-08-15.md` §10。
- **OMP advisor 启用（强模型审查弱模型）**: `modelRoles.advisor = zg-newapi-anthropic/claude-opus-5:high` + `advisor.enabled = true`，opus 逐 turn 旁听审查 k3 主会话。无头会话实测 `__advisor.jsonl` 生成、审查闭环正常。定位是事后纠错安全网非能力增强；slow 档流量预期翻倍，观察干预频率后决定是否降级为仅 task 子代理。prewalk（反向省成本杠杆）未开，避免干扰观察。详见 `docs/ops/agnes-25-models-vision-role-2026-08-14.md` §8。
- **Agnes 2.5 模型暴露 + vision 角色降本**: 直查 agnes 上游发现 `agnes-2.5-pro`/`agnes-2.5-flash` 在售但未接入。NewAPI ch68/69 models 补齐两模型 + 配价（0.5/2）+ abilities 重建（34/0）；OMP models.yml 新增 3 个 agnes-2.5 条目（均实测支持识图）。`vision` 角色从 `claude-opus-5` 降为 `zg-newapi/agnes-2.5-pro`（免费池），fallback 链 qwen3.8-max 出、`agnes-2.5-flash` 入；dots-3-note-prev（ch77）实测连续 429 故不作主。fork API 约束复踩：本机 fork `PUT /api/channel` 报 `Invalid parameters`，改走 DB 直改 models 列 + `/api/channel/fix`。详见 `docs/ops/agnes-25-models-vision-role-2026-08-14.md`。
...
- **NewAPI 个人使用调优（P0/P1）**: 基于 GitHub 源码和社区最佳实践调整。**P0**：`ChannelDisableThreshold: 10→3`（个人使用渠道少，更快剔除故障渠道）、`RetryTimes: 4→2`（重试次数减少，更快失败）、`AutomaticRetryStatusCodes` 移除 402（余额不足重试无意义，应立即 fallback）。**P1**：`monitor_setting.auto_test_channel_minutes: 10→30`（个人使用渠道少，减少无效测试）、`monitor_setting.channel_test_mode: scheduled_all→scheduled_enabled`（只测试 enabled 渠道）、`general_setting.ping_interval_seconds: 60→30`（SSE 保活间隔减半，减少长推理断连）。验证全部生效。参考：`controller/channel.go` `UpdateChannel`（status 禁止）、`service/channel.go` `ShouldDisableChannel`（自动禁用逻辑）、DeepWiki `3.4-automatic-channel-testing`（测试模式）。

- **Guardian 完整修复（P0/P1 + atomcode 支持）**: `scripts/ops/guardian.py` 深度修复。**P0**：Telegram 命令处理（/status /channels /report /restart /enable /disable /help，getUpdates 轮询 + 命令解析）、渠道自动启用（`check_and_enable_recovered_channels()` 禁用后自动恢复）。**P1**：渠道健康检查完善（`test_channel` 发送真实请求验证，`GET /api/channel/test/{id}`）、本地代理重启完善（agentrouter/codebuddy/anyrouter/atomcode 各自启动参数，atomcode 用 Node.js）、NewAPI 重启完善（SSH/本地 podman 多种方式 + 错误处理）、错误率计算准确化（过滤 channel_id/model_name）、报警冷却按级别（error=1min, warning=5min, info=30min）、本地代理测试修复（anyrouter /health, atomcode /v1/usage, 正确 API key）。**atomcode 支持**：添加 atomgit-opencode-bridge 到监控和重启（`C:/Users/zhugu/atomgit-opencode-bridge`，Node.js `proxy.js`，端口 9457）。

- **agentrouter-proxy 安全/重试修复（8 项）**: 深度审查 + 修复 `agentrouter-proxy.py`（三个审查代理并行）。**① BLOCKER** 旧进程 `--host 0.0.0.0` 无 `--api-key` 暴露给 LAN/Tailscale → 杀进程 + 重启显式绑定 127.0.0.1 + watchdog 加 `--host 127.0.0.1`；**② HIGH** 500 在重试集合致 4key×2域=16 次上游放大（NewAPI 预扣费 per request，失败后仍计费）→ `_is_retryable_status` 移除 500（保留 502/503/504/52x 网关级）；**③ HIGH** 重试无总时限最坏 ~84min → 加 `_DEADLINE_S=120` 总时限超时返 502；**④ MED** `raise HTTPException(detail=...)` 包 `detail` 致 OpenAI 客户端读不到顶层 `error` → 加 `@app.exception_handler(HTTPException)` 展平 + 4 处改 `JSONResponse`；**⑤ MED** `keys.json` ACL 继承给 CodexSandboxUsers → icacls 收收紧至仅 zhugu。验证：500 不重试、错误体顶层可达、127.0.0.1 绑定生效 Tailscale 不可达。详见 `docs/patches/agentrouter-proxy-security-fix-2026-08-02.md`。
- **deepseek-v4-flash 聚合池慢源治理终版**: 在既有按延迟重排权重基础上，**禁用毒瘤慢源 ch55/ch50（inferx，avg 48-65s）**（status=2 + abilities.enabled=0 双保险），14 源→12 源；清理 ch35 重复 abilities 条目（`deepseek/deepseek-v4-flash` 别名删，保留裸名 weight=18）。重启后 20 发实测：快源主导（ch37:5 ch38:4），整体 avg ~2.5s（此前 ~27s），慢源零命中。聚合池快照已同步更新。
...
- **claude 聚合池七源→二源重组 + 禁用真正生效修复**: 深度排查发现 claude 请求近 6h 全部命中 gorouter（ch26/27/28）、ch3/9/18（baibei/linxi-k40）被排除，直测三源全部 503 `All available accounts exhausted`（上游账户耗尽）→ 禁用手动 ban。gorouter 同上游三 key 合并为 ch57（weight=4），agentrouter ch45 权重 5→15 成主源（15/19≈79%）。**重大修复**：NewAPI 渠道禁用必须 status=2 + abilities.enabled=0 双保险——之前 ch16/25/43 的 status=0 禁用（`ChannelStatusUnknown`）从未生效，路由过滤实际看 abilities 表。**多 key 渠道三坑**：key 必须真实换行（bash 写 `\n` 变字面量）、`channel_info` 必须 bytes 写入（str 被 GORM 二次编码致 `unexpected end of JSON input`）、`multi_key_mode` 必须字符串 `"polling"`（数字解码失败）。修复后 claude 双源按权重 15:4 分流，零错误。
...
- **HF Space share-api DeepSeek 免费 key（ch58）**: `2c2ch1u11-share-api-0.hf.space`（base_url **不带 /v1**——初设带 `/v1` 致 NewAPI 拼 `/v1/v1/...` 404，去 `/v1` 后修复，仅余 429 限流），模型 `deepseek-v4-flash`，key `sk-82de10c1-...`（20260801）。120 RPM/key 严格限流、上游响应慢（首次 60s+ 冷启动）、上下文非 1M。weight=2 极低分流，deepseek 池十四源。验证 `finish:stop CH58-OK`。
- **ch30 fastaitoken gpt-5.6-sol abilities 修复**: code review 发现 ch30（付费主源）gpt-5.6-sol 近 6h 零命中，而免费本机源 ch44/45 各 246/171 次——根因是 abilities 表 gpt-5.6-sol 的 priority 被降到 10（低于 ch44/45 的 50）→ 路由永不选 ch30。恢复 priority=50、weight=20 后 12 发分布 ch30=9/ch44=3/ch45=3（主源回归主导）。**教训**：NewAPI 路由按 abilities 表的 weight/priority（每模型独立），channels.weight 只是默认值——排查“某渠道不命中”必须查 abilities。
- **ch58 base_url 去 /v1 修复 + ch15 u1-fast 一致性清理**: 深度排查发现 ch58 初设 base_url 带 `/v1` 被 NewAPI 拼成 `/v1/v1/...` 返 404（去 `/v1` 后仅余 429 限流，符合免费源预期）；ch15 的 abilities 残留 `sensenova-u1-fast`（channels.models 已移除该模型、近24h 零使用）→ 禁用 abilities 保持一致。全量检查确认：无孤儿渠道、无启用无 abilities 渠道、禁用渠道近30min 零命中。
...
- **NewAPI DeepSeek 官方直连渠道（ch42）+ 官方/聚合分离**: `docs/patches/newapi-deepseek-official-channel-2026-07-31.md` — DeepSeek 官方 API 接入 ch42（type=1，sqlite 直写 + BLOB channel_info），用 `model_mapping` 把 `deepseek-official-v4-flash`/`deepseek-official-v4-pro` 映射到上游真名；官方别名只走 ch42，裸名 `deepseek-v4-flash` 只走聚合池（ch15/40/126），互不打散缓存。已配 Kimi/OMP 官方别名入口；deepseek 亲和规则（rule #4，UA fallback）自动覆盖。定价已复制到官方别名。备份 `one-api.before-ch42-official-split-20260731-182419.db`。
- **LongCat 官方直连接入 OMP + Kimi**: `docs/patches/longcat-official-omp-kimi-2026-07-31.md` — 美团 LongCat 官方 API（`api.longcat.chat/openai/v1`，注意 `longcat.chat` 主站是网页网关非 API）接入 OMP/Kimi：`LongCat-2.0` 1M ctx/131K out/text-only，默认带 reasoning（`reasoning_effort: minimal` 可关，实测出 content），原生 `/responses` 亦通。按用户选择只接本地客户端不动 NewAPI（VPS 现无 LongCat 渠道，Claude Haiku 角色仍走 ch122 Agnes 中转）。两端 CLI 冒烟 OK。
- **官方 deepseek-v4-flash 计费修正（CacheRatio 0.25→0.02，~38× 降）**: `docs/patches/newapi-deepseek-official-channel-2026-07-31.md` §4.2 — 官方别名定价原复制旧 deepseek-chat 比例（hit/miss=0.25）；V4 Flash 官方现行价 miss $0.14/M、hit $0.0028/M（2%）→ 全量对齐 ModelRatio 0.07 / CompletionRatio 2 / CacheRatio 0.02 / CreateCacheRatio 1.0。logs quota 反推确认运行计费（0.5/0.25/2/1.0）逐行吻合；发现 options DB 与内存漂移（CCR DB=1.25 运行=1.0，重启会静默变价）一并对齐。备份 `one-api.before-official-flash-pricing-20260731-184412.db`；改后同前缀请求仍命中缓存（cached=2432），新 quota=9 vs 改前 342.5。
- **WorkBuddy DeepSeek V4 Flash 本地代理接入（OMP + Kimi）**: `docs/patches/workbuddy-deepseek-v4-flash-omp-kimi-2026-07-31.md` — WorkBuddy（CodeBuddy 后端）`deepseek-v4-flash` 经 `codebuddy2openai`（127.0.0.1:8787）暴露；转换器 `DEFAULT_MODELS` 本就含该模型，无需改，仅补 OMP/Kimi 别名（1M 上下文 / 32K 输出）。实测非流式/流式/两端 CLI 全 OK；与 NewAPI 官方 ch42、聚合池、Cline 池三来源互相独立。
...
- **本机 Cline 清理，仅保留 VPS 池**: `docs/patches/local-cline-cleanup-vps-pool-2026-07-31.md` — 删除本机 CLI、账户数据和 `127.0.0.1:3457` 代理；OMP/Kimi 仅保留 NewAPI/VPS Cline 池，WorkBuddy `8787` 不受影响。
- **WorkBuddy GPT 5.6 Sol 转换器接入 + 多 key 健康冷却**: `docs/patches/workbuddy-gpt-sol-converter-keypool-2026-07-31.md` — 第三方 `gpt-5.6-sol`（freemodel.dev）经 `codebuddy2openai` 接入 OMP/Kimi。根因是网关按请求头路由（需 WorkBuddy Electron UA+Referer+Origin）且 key 绑定后端分片（实测 4 key 中 2 个分片死）；转换器加请求头适配 + 按 key 健康冷却（死 key 冷却跳过、恢复自愈），非流式 8/8、流式 OK。代码审查后加固 9 项：流式错误分类统一（400/401/403/404 不再误重试）、完整 payload/SSE 不再落日志且流式有限超时、冷却并发竞态按 req_epoch 判定、URL 强制 https 拒私网、custom_keys 无效值回退、空流不丢 chunk、配置按 mtime 热加载、/health 鉴权脱敏。
- **Step Plan step-router-v1 接入 NewAPI（ch36）**: `docs/patches/stepfun-step-plan-newapi-2026-07-31.md` — 阶跃 Step Plan 通道 `step-router-v1`（触达 deepseek-v4-pro 的唯一途径，deepseek-v4-pro 本身不可直连）接入 NewAPI 渠道 36；base_url 不带 /v1、channel_info BLOB、定价 0.5/2.0；经公网 5/5 + 流式 OK。
- **InferX 免费源接入（ch49 glm-52 + ch50 deepseek-v4-flash-0731）**: `model.inferx.net/endpoints/v1`（InferX serverless GPU，每 100 万 token 免费，deepseek-v4-flash-0731 产出 0 美元）。ch49 挂 `glm-5.2`（model_mapping→`glm-52`，上游 `cyankiwi/GLM-5.2-AWQ-INT4`）进 glm-5.2 池六源；ch50 挂 `deepseek-v4-flash`（mapping→`deepseek-v4-flash-0731`）进 deepseek 池十源。weight=5 补充定位、auto_ban=0（免费源容量间歇 429 capacity）。验证：glm `finish:stop content:GLM-IX-OK`、deepseek `finish:stop content:DS-IX-OK`。
- **AtomCode CodingPlan Lite deepseek-v4-flash 接入 NewAPI 聚合池（ch43）**: `docs/patches/atomcode-cp-lite-newapi-2026-07-31.md` — agentrouter/anyrouter 公益池间歇 503 后补量，VPS 部署 AtomCode 5.0.3 + `atomcode-open-api` 代理（systemd、0.0.0.0:8899、podman 桥 10.88.0.1 可达），`atomcode login` 激活 CodingPlan Lite（800 次/5h 免费额度，token 7 天过期），ch43 进 `deepseek-v4-flash` 聚合池（五源分流：ch15/35/37/38/43），隔离冒烟验证 finish=stop content=ATOMCODE-OK。
- **atomcode ch43 deepseek 根因纠正 + 退出 deepseek 池**: 之前判定"每月token额度打满"是**错误的**——status-v2 显示 `calls_used:0 / usage_percent:0`（额度根本没消耗）。真根因是上游双网关策略：旧网关 `api-ai.gitcode.com` 对 deepseek 返回业务 403 `model is not enabled for codingplan 'Lite'`（Lite 档不启用 deepseek），新网关 `llm-api.atomgit.com` 要求真客户端签名（代理签名被拒 `SIG_MISSING`）。代理补了真实签名 + UA(`atomcode/5.0.3`) + `x-atomcode-session-id` 后，旧网关不再伪装"每月token额度已不足"而是暴露真 403。结论：ch43 对 deepseek 不可用（上游业务决策 + 签名墙），已从 deepseek 聚合池摘除（abilities enabled=0 + models 清空），deepseek 靠其他 9 源（5/5 正常）。Qwen3-VL 纯文本在旧网关可用（签名修复后），图片输入仍被上游"独享"校验拦截。
- **atomcode 代理拦截"每月token额度已不足"陷阱**: 根因——ch43 atomcode 代理上游（CodingPlan LLM 网关）额度耗尽时返回 200 OK 但 content 是"每月token额度已不足"，不是 HTTP 错误，NewAPI 透传，pi 当模型回复展示。修复：atomcode 代理 `server.py` 非流式+流式均检测该文案，命中时转为 HTTP 429 quota_exhausted 错误，NewAPI 按 429 触发 failover 到其他 9 个渠道。验证：非流式→429、流式→error SSE、聚合池 failover 正常。
- **402 加入 NewAPI 重试状态码**: bazaarlink（ch46/47）免费额度打满返 402 `Insufficient credits`，但 402 不在 `AutomaticRetryStatusCodes` → 不 failover 直接返给客户端 → OMP 报 402。修复：`AutomaticRetryStatusCodes` 加 402（额度打满的源 402 触发重试/切其他源）。ch46/47 auto_ban=0 保持（免费源突发 429 常态，聚合池文档有意为之）。验证聚合池 3/3 正常。
- **NewAPI auto_ban 阈值 50→3 + sensenova 月度配额打满自动禁用/恢复**: 排查"每月token额度已不足"——根因是 ch15 sensenova（商汤）月度配额打满返 429，但 `ChannelDisableThreshold=50` 太高（logs 表只积累 3 次）→ 渠道不禁用继续被路由 → OMP 轮到时撞 429 报错。修复：`ChannelDisableThreshold` 50→3（打满 3 次即禁），`AutomaticDisableChannelEnabled` + `AutomaticEnableChannelEnabled` 均 true（禁后额度恢复自动拉回）。修复闭环：sensenova 打满→3次429自动禁→failover 其他9源→额度恢复自动启用。验证聚合池 3/3 正常。
- **OpenCode Go deepseek-v4-flash 接入聚合池（ch48）**: `https://opencode.ai/zen/go`（NewAPI type=1 自动补 `/v1/chat/completions`，base_url 带 `/v1` 会拼成 `/zen/go/v1/v1/...` 返 404）。裸名 `deepseek-v4-flash` 直通，支持缓存（prompt_cache_hit_tokens）。$10/月订阅源 weight=5 auto_ban=0。隔离验证 `finish:stop content:OG-OK`，deepseek-v4-flash 聚合池扩至十源。
- **centos.hk 欠费摘除（ch16/25）+ luna/terra 下架**: centos 上游账户欠费返 403「预扣费额度失败 用户剩余额度 ¥0.09」（该"用户"指 centos 账户，**非**本地无限钱包 user1 quota=9994亿）；NewAPI 对上游 403 默认不 failover，故双保险摘除（status=0 + abilities enabled=0）。gpt-5.6-sol 剩 ch30/44/45 三源、gpt-5.5/image-2 剩 ch30 单源；gpt-5.6-luna/terra 仅挂 centos 摘除后零源，已从 OMP/Kimi 配置移除。聚合池文档 §3 同步。
- **linxi-k40/baibei claude 保守后备管理（ch3/9/18）**: 三渠道本就在 claude 聚合池且 priority 最高（57/54），但 type=14 致 OpenAI 格式测试看不到（误以为没入池）。priority 降至 30 作后备（稳定靠高 weight 吃量、挂时不优先吸流量）；新增 `/opt/new-api/k40-baibei-revive.py` + systemd timer 每分钟赦免 auto_ban（status=3→1），SQL 带 `AND status=3` 不碰手动禁用（status=2），判活交 NewAPI 下轮测试零误判。端到端验证：模拟 ch3 status=3 → 脚本赦免回 1。亲和已全局去除（enabled=false）确认无粘滞。
- **BazaarLink 免费 deepseek-v4-flash 接入聚合池（ch46/47 双 key）**: 两 key 各建单渠道（避开 NewAPI 多 key 换行 header 坑），base_url `https://bazaarlink.ai/api`（**非** `/api/v1`，NewAPI type=1 自动补 `/v1`，初设带 /v1 会拼成 `/api/v1/v1/...` 返 404）。免费 10 RPM/150 每日加权扣量，auto_ban=0 防突发 429 误杀，weight=3 补量定位。隔离验证 `finish:stop content:BZ-OK`，deepseek-v4-flash 聚合池扩至九源（ch15/35/37/38/42/43/44/46/47）。
- **cline 代理工具调用补丁持久化**: 写幂等 `patch_tools_passthrough.py` 挂 cline-proxy.service `ExecStartPre`（node --check 校验 + 失败回滚 .bak），源码还原后下次启动自动重打。生产级验证：含补丁态重启 ExecStartPre 静默幂等通过、服务 active、带 tools 请求 finish:tool_calls。
- **cline 代理工具调用失败修复**: `cline-glm-proxy.mjs` 的 `callClineDirect` 构造上游 body 时只取 model/messages/max_tokens，**丢弃 tools/tool_choice** → 上游 cline gateway 收到带 tool_calls 历史却无工具 schema 的请求 → `empty response content` / 500，工具调用全失败。修复透传 `params.tools`/`params.tool_choice`，验证带 tools 请求返回 `finish:tool_calls` + 正确函数调用。
- **atomcode 流式漏计费补丁持久化**: 写幂等 `patch_stream_usage.py` 挂 systemd `ExecStartPre`，pip 升级/源码还原后下次启动自动重打（已验证幂等 exit 0 + 剥离后重打 applied+compile ok）。
- **atomcode 流式 completion_tokens=0 漏计费修复**: atomcode 代理流式透传未注入 `stream_options.include_usage`，DeepSeek 流式默认不返 usage → NewAPI 记 completion=0、completion 漏计费（ch43 近30min 54命中50次零completion，其他渠道全正常）。`gateway.py` 加 `_inject_stream_usage` 流式转发前注入，验证流末返 `completion_tokens:17`、隔离请求日志 completion=23 非零。
- **atomcode token 失效 + ch15/35/37/38/44 deepseek ability 漏恢复修复**: atomcode access_token 实际失效（health 显示 6d 21h 但请求 401），手动 refresh 恢复；隔离验证时禁用 ch15/35/37/38/44 的 deepseek-v4-flash ability 后恢复命令漏了渠道致 25 分钟无流量，全量核对后恢复七源。
- **NewAPI 聚合池总览（防漂移快照）**: `docs/patches/newapi-aggregation-pools-2026-08-01.md` — deepseek-v4-flash 七源（ch15/35/37/38/42/43/44）、glm-5.2 五源（ch14/35/37/38/44）、gpt-5.6-sol 五源（ch16/25/30/44/45）、claude-opus-5/4-8 四源（ch26/27/28/45），含权重/auto_ban/备注。记录 claude 亲和移除、亲和系统关闭、gorouter type 修复、ch18 禁用、ch34 删除。此后任何聚合池变更须同步该文档。
- **code review 修复（agentrouter 403 全量冷却 + atomcode tomllib 兜底）**: agentrouter-proxy 原先只对 quota 403 冷却换 key，key 失效/被删的 403 仍直接中止请求 → 改为所有 403 都冷却 3600s 并换下一个 key（防 4 key 池中死 key 拖垮全部请求）；atomcode auth.py save_auth 在无 tomllib 的 Python 3.10 下会丢 user 段 → 加 `_parse_auth_simple_user` 兜底解析。两代理已重启验证。
- **gorouter 渠道 type=14→1 修复（定时测试失败致路由跳过）**: gorouter 三渠道（ch26/27/28）原 type=14（Anthropic），NewAPI 定时测试用 OpenAI 格式测返回空响应 → 内存标记降级 → 路由时跳过，全走 ch45。改 type=1 + 重建容器清缓存后恢复正常分流。
- **移除 claude 亲和性规则（ch26/27/28 gorouter 按权重分流）**: 删除 `channel_affinity_setting.rules` 中的 `claude cli trace`，Claude 模型改为纯权重轮询。gorouter 三渠道（ch26/27/28）+ agentrouter（ch45）均匀分流，避免粘滞到单渠道。codex/glm/grok/deepseek/longcat/qwen 六条规则保留。
- **auto_ban 策略优化**: 本机源（ch44 codebuddy、ch45 agentrouter）关 `auto_ban`（依赖本机桌面，断连是常态）；VPS 源（ch35 cline-free、ch43 atomcode）保持 auto_ban=1（VPS 稳定，真挂该禁）。付费源（ch14/15/37/38/42）保持。
- **ch18 linxi-k40 禁用（502 不稳定）**: Claude 付费中转 linxi-k40 频发 502，禁用后 claude-opus-5/4-8 由 gorouter(ch26/27/28) + agentrouter(ch45) 覆盖，claude-opus-4-7 无替代源，已从 OMP/Kimi 配置中移除。
- **agentrouter 代理 403 配额不足冷却修复**: 本机 agentrouter-proxy 4 key 池中某 key 余额不足时返回 403，代理未冷却该 key 直接抛给 NewAPI 变 500。新增 `_is_quota_403()` 检测 + 3600s 冷却 + 自动换下一个活 key。
- **ch35/cline-free 和 ch42/official models 字段补裸名**: ch35 缺 `deepseek-v4-flash,glm-5.2`，ch42 缺 `deepseek-v4-flash`，导致 NewAPI 按 `models` 列表过滤时路由不到聚合池。已补全。
- **anyrouter 上游调查（1M 上下文 + 503 容量）**: `docs/patches/anyrouter-1m-context-503-2026-07-31.md` — 发现代理认证方式错误（`x-api-key` → `Authorization: Bearer` 才正确）；上游强制 1M 上下文但带 header 后 503（Claude 池无容量）；暂不接入 NewAPI。
- **ch45 agentrouter 进 claude-opus-5/4-8 聚合池**: ch45 扩 `claude-opus-5,claude-opus-4-8`，claude-opus-5 五源（ch18/26/27/28/45），claude-opus-4-8 六源（ch18/26/27/28/45）。隔离验证 `AR-OPUS-OK` / `AR-OPUS48-OK`。
- **ch45 agentrouter 进 gpt-5.6-sol 聚合池**: 本机 agentrouter-proxy 经 Tailscale 接入 NewAPI（ch45，--host 0.0.0.0），gpt-5.6-sol 六源（ch16/25/30/34/44/45），隔离验证 `AR-GPT-OK`。
- **ch44 codebuddy 进 gpt-5.6-sol 聚合池**: ch44 加 `gpt-5.6-sol` 模型 + ability，gpt-5.6-sol 五源（ch16/25/30/34/44），隔离验证 `CB-GPT-OK`。
- **ch44 codebuddy 本地代理进聚合池（glm-5.2 + deepseek-v4-flash）**: 本机 codebuddy2openai converter 经 Tailscale（100.83.32.95:8787）接入 NewAPI。converter 改 `--host 0.0.0.0` 放通外网访问，ch44 weight=5（低于 VPS 源的 10，防本机掉线影响）。隔离验证 `CODEBUDDY-OK` / `CB-FLASH-OK`。
- **glm-5.2 聚合池**：ch14/35/37/38/44 五源
- **deepseek-v4-flash 聚合池**：ch15/35/37/38/42/43/44 七源（含 DeepSeek 官方直连）
- **ch35 cline-free 加入 glm-5.2 聚合池**: ch35 model_mapping 追加 `"glm-5.2":"cline-free/glm-5.2"`，glm-5.2 聚合池四源（ch14/35/37/38），隔离验证 `GLM-CLINE-OK`。更新 OMP/Kimi 模型名。
- **ch35 cline-free 加入 deepseek-v4-flash 聚合池**: 裸名 `deepseek-v4-flash` 经 model_mapping 映射到 `deepseek/deepseek-v4-flash`，聚合池扩容五源，验证 `CLINE-FREE-OK`。更新 OMP/Kimi 模型名。
- **OMP fallbackChains 重构（router 互备 + vision/tiny 覆盖）**: `docs/patches/omp-fallback-chains-2026-07-31.md` — OMP `config.yml` 四链重写并加入 `anyrouter/*` → `agentrouter/*` → `zg-newapi-anthropic/*`、`agentrouter/*` → `zg-newapi-anthropic/*` 通配链。`modelRoles.vision = agentrouter/claude-opus-5:high` 只让 `inspect_image` 选择 vision-capable 模型；该工具自身不经过 `TurnRecovery`、无 retry/fallback，`fallbackChains.vision` 只保护主会话 `@vision` assistant 轮次。隔离 overlay（重试 1 次、100ms/5000ms 退避）下，8789 模型请求 16.87 秒返回 502（上游 503），agentrouter 定向 OMP 请求 9.50 秒成功，但两次 anyrouter OMP 请求均在 90/115 秒 deadline 前未切换，故 wildcard E2E 仍未证明；按门禁未持久写入全局 `maxRetries: 3`，有效值仍为默认 10（默认退避 500ms/300000ms）。
- **NewAPI prompt cache 计费补全**: `docs/patches/newapi-prompt-cache-ratio-2026-07-31.md` — OMP 6 模型（k3/step-router-v1/qwen3.8-max-preview/cline-free/glm-5.2/poolside/laguna-s-2.1:free/deepseek/deepseek-v4-flash）上游支持 prompt caching 但 NewAPI CacheRatio/CreateCacheRatio 未配，缓存命中按全额计费；补全 CacheRatio 0.1~0.25 / CreateCacheRatio 1.25，k3 二次请求 cached_tokens=178 命中生效。abilities 14/14 齐全（此前为 shell 引号 bug 误判）。
- **NewAPI 渠道亲和性修复（OMP 非 claude 模型）**: `docs/patches/newapi-channel-affinity-2026-07-31.md` — 亲和规则 key 源原为请求体 prompt_cache_key（OMP 不发），导致同 token 请求打散到 8 个渠道、缓存命不中；给 codex/glm/grok/qwen/deepseek/longcat 6 规则追加 User-Agent fallback key 源 + 启用 glm trace，glm-5.2 验证粘滞 ch14。关联发现 gpt-5.6-sol 的 ch16/30 成功率 0%（auto_ban 未触发）且原权重最高反分最多流量，已降为兜底（priority 10）、ch34 提为主渠道（priority 50），近窗 7/7 成功。
- **agentrouter 本地代理接入 OMP/Kimi（claude-opus-5）**: `docs/patches/agentrouter-local-proxy-2026-07-31.md` — agentrouter 从 VPS 不可达（原域名网络封 + 备用域名 WAF 挡数据中心 IP）故不接 NewAPI，本机起代理（127.0.0.1:8788）注入 claude-cli UA 转发；4 key 池 + 429 冷却抗限流、域名 failover、过滤 data:null 规范化 SSE 帧（修 Kimi null.id 崩溃）；OMP/Kimi 均验证 claude-opus-5 成功，watchdog 自启。
- **tokenrhythm GLM-5.2 接入 NewAPI（glm-5.2 去单点）**: `docs/patches/tokenrhythm-glm52-newapi-2026-07-31.md` — glm-5.2 原仅 ch14 wintoken 单源（SPOF），新增 tokenrhythm.studio 两渠道 ch37/38（2 key 拆分，pri50/w10），三源按权重分流 + glm trace 亲和；6/6 成功、最近窗 ch14/37/38 全 0 失败。大陆优化后端 fcapp.run 账户欠费（403）不接。
- **anyrouter.top 接入 OMP/Kimi（Claude Code 指纹破解）**: `docs/patches/anyrouter-claude-fingerprint-2026-07-31.md` — 破解 anyrouter opus 的 Claude Code 客户端指纹校验（anthropic-beta 含 claude-code-20250219/context-1m + claude-cli UA + X-Stainless-* + metadata.user_id，来自社区 relay-pulse 模板）；本机 Anthropic 透传代理（8789）注入指纹 + key 轮换，接入 OMP/Kimi（1M 上下文）。穷尽验证（含官方 SDK）当前 503 为上游 claude-code 池中断（公益站间歇），代理已就绪、恢复后自动可用。
- **ai.168661 grok-4.5 接入 NewAPI（ch39）**: `docs/patches/ai-168661-grok45-newapi-2026-07-31.md` — ai.168661.xyz 已恢复提供 grok-4.5（AGENTS.md 旧记停供为旧情况），VPS 可达、带 reasoning+缓存命中；接 ch39（w10 备级，auto_ban 管限流期）。grok-4.5 三活跃源（ch29 w20 + ch17 w10 + ch39 w10）；key 级上游限流为瞬时突发（~60s 恢复），failover 兜底正常。
- **WorkBuddy sol 上下文虚高修复 + agentrouter 诊断**: `docs/patches/workbuddy-sol-context-fix-agentrouter-diag-2026-07-31.md` — gpt-5.6-sol 不稳定根因是 max_context 误配 1M 超 freemodel 上游 ~343K 实际窗口（agent 526 消息会话超限报错），Kimi/OMP 均降为 256K（agent 提前压缩避开超限与慢速区）；agentrouter 不稳为上游间歇限流 429+内容拦截（key 池缓解，当前 6/6 稳定）；glm-5.2 走 CodeBuddy 官方后端真 1M 不虚高无需改。
- **NewAPI glm-5.2 prompt_cache_key 400 修复（订正根因）**: `docs/patches/newapi-glm-prompt-cache-key-400-fix-2026-07-31.md` — 真根因：客户端（kimi/omp 开缓存）在请求体直接发 prompt_cache_key 非标准字段，NewAPI 原样透传，tokenrhythm（ch37/38）严格拒该字段报 400。修复：ch37/38 的 param_override 高级模式 `mode:delete` 剥离该字段，A/B 验证 18/18 全 200。推翻旧判定（误以为亲和规则注入，实为客户端自带字段透传）。
- **NewAPI 0v0.club glm-5.2 备份渠道（ch40）**: `docs/patches/newapi-0v0-glm-backup-2026-07-31.md` — 0v0.club 接 ch40 作 glm-5.2 第四源（w5），与 wintoken/tokenrhythm 共四源；后扩挂 deepseek-v4-flash。注意 rc.21 多 key 模式不可靠（换行泄漏到 Authorization header），ch40 改单 key。
- **NewAPI ch15 sensenova-u1-fast 错挂摘除**: `docs/patches/newapi-sensenova-u1-fast-disable-2026-07-31.md` — u1-fast 是图像生成模型（output=image）不可走 chat completions，上游 /models 误导性列出，NewAPI 当 chat 挂致历史 190 条 404。禁用 ability + 移出 channels.models。
- **NewAPI deepseek-v4-flash 503 救火 + 单点根治**: `docs/patches/newapi-deepseek-v4-flash-503-rescue-2026-07-31.md` — ch15 sensenova 两 key 被 429 永久 auto_ban（不自动恢复）致裸名 deepseek-v4-flash 单点全挂；救火踩 rc.21 多 key 换行 header 坑。修复：ch15/ch40 改单 key、重置 channel_info+status+abilities、ch40 扩挂 deepseek-v4-flash 补源（双源）。auto_ban 永久性 + 多 key 不可靠两项教训。
- **NewAPI zzzcoding.org gpt 备份渠道（ch41）**: `docs/patches/newapi-zzzcoding-gpt-backup-2026-07-31.md` — zzzcoding.org 接 ch41（w5），挂 gpt-5.5/5.6-sol/5.6-luna/5.6-terra/5.4/5.4-mini 六模型；重点补 gpt-5.6-luna 单点（ch16→16+41 双源）。排除 gpt-image-2（图像模型不可 chat，同 u1-fast 坑）和 gpt-5.3-codex/5.2（502）。单 key 模式。
- **NewAPI 渠道列表前端"未找到渠道"修复**: `docs/patches/newapi-channel-info-blob-fix-2026-07-31.md` — ch15/40/41 的 `channel_info` 被以 TEXT/NULL 写入；Go `ChannelInfo.Scan` 只接受 BLOB `[]byte`，类型断言失败后反序列化 `nil`，致 `GET /api/channel` 失败、前端列表为空。修复流程现包含停机备份、NULL 单 key 回填、合法 TEXT→BLOB、完整性/类型/JSON 校验与回滚；本次三个单 key 渠道实测 relay 不受影响，但多 key 渠道不能作同样推断。
...
- **NewAPI anti-stall automation**: `docs/patches/newapi-dx-anti-stall-2026-07-26.md` — demote `#20/#60`, park AR `#119/#120` Opus; analyze latency no-raise/FORCE_DEMOTE; schtask every 4h.
- **Local guard-gap policy**: keep FQ ZG→`agentrouter-2`; lock 林夕/Sub2API/百倍 out of FQ/current (VPS kiro-guard covers ZG path only; no local guard, FQ#2 not redirected to ZG).
- **NewAPI health_check v5**: `docs/patches/newapi-dx-health-check-v5-2026-07-26.md` + `scripts/ops/health_check.vps.py` — Opus-pool-only probes; no auto-reactivate / DISABLE-QUOTA / priority writes / blanket abilities; 降智探针 off.
- **NewAPI AR pin**: `docs/patches/newapi-dx-ar-pin-2026-07-26.md` — after top-up restore `#118` w6; **`#119/#120` stay closed for now** (anti-stall).
...
- **Restore reconcile no longer strips a hand-authored `[thinking]` table from Kimi Code live config**: stripping is now gated on the user having explicitly cleared the DB snippet (`snippet_cleared`), preserving takeover losslessness; the legacy `proxy_config` migration also seeds the `reasonix` row so old installs match the per-app schema's CHECK constraint.
- **Kimi common-config editor re-syncs from the backend after a failed save**: a partially persisted snippet (DB saved, backup write failed) is no longer overwritten by a stale local draft on reopen, and the modal only seeds its draft on the closed→open transition so a save-failure refetch cannot clobber in-flight edits. Entry button and error/guide blocks also restyled.
- **Reasonix takeover under `network.proxy_mode = "custom"` injects loopback into `[network].no_proxy`** so the local ingress (`http://127.0.0.1:<port>`) cannot be captured by the user's proxy — the pre-takeover value is stashed once per session and replayed on clear (full-snapshot restores discard the stash), and the proxy panel now shows a live warning if a manual edit removes the bypass. Detection only trusts a literal `127.0.0.1` entry (mainstream clients match `no_proxy` literally, no DNS resolution), closing the localhost-only false negative.
- **Codex→Chat/Anthropic relay clamps `max_output_tokens` to the provider ceiling** (`clamp_codex_max_output_tokens`): provider `meta.max_output_tokens` wins; otherwise implausibly large values (>131072 — e.g. a client deriving it from context size like Kimi Code's 262144) are clamped to 131072, fixing hard-400 `InvalidParameter: max_tokens` rejections from gateways such as capi.cun.ai.
