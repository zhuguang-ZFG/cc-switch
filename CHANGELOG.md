...
- **NewAPI glm-5.2 prompt_cache_key 400 修复**: `docs/patches/newapi-glm-prompt-cache-key-400-fix-2026-07-31.md` — 亲和性 glm trace 规则的 prompt_cache_key key_source 透传该字段到上游，新接入的 tokenrhythm（ch37/38）严格不认非标准字段报 400（wintoken ch14 兼容故不报）；移除 glm trace 的 prompt_cache_key key_source 只留 User-Agent（亲和仍粘滞），5/5 成功。
