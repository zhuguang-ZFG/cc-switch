...
- **NewAPI 渠道列表前端"未找到渠道"修复**: `docs/patches/newapi-channel-info-blob-fix-2026-07-31.md` — ch15/40/41 的 channel_info 被 Python sqlite3 写成 TEXT 类型（其余是 BLOB），GORM json.RawMessage 扫描 TEXT 报 "unexpected end of JSON input"，致 GET /api/channel 序列化失败、前端渠道列表空。修复：CAST(channel_info AS BLOB) 归一类型。教训：直写 DB 后用 CAST 转 BLOB，勿留 NULL/TEXT；relay 转发不受影响只影响后台显示。
