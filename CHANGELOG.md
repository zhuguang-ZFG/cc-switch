...
- **WorkBuddy GPT 5.6 Sol 转换器接入 + 多 key 健康冷却**: `docs/patches/workbuddy-gpt-sol-converter-keypool-2026-07-31.md` — 第三方 `gpt-5.6-sol`（freemodel.dev）经 `codebuddy2openai` 接入 OMP/Kimi。根因是网关按请求头路由（需 WorkBuddy Electron UA+Referer+Origin）且 key 绑定后端分片（实测 4 key 中 2 个分片死）；转换器加请求头适配 + 按 key 健康冷却（死 key 冷却跳过、恢复自愈），非流式 8/8、流式 OK。
