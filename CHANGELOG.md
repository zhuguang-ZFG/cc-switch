...
- **OMP fallbackChains 重构（router 互备 + vision/tiny 覆盖）**: `docs/patches/omp-fallback-chains-2026-07-31.md` — OMP `config.yml` 四链重写：`default` 从纯 deepseek 改为 agentrouter 互备优先（`agentrouter/claude-opus-5` → `agentrouter/claude-opus-4-8` → `zg-newapi-anthropic/claude-opus-5` → deepseek 兜底）；`slow` 尾部加 agentrouter 后备；新加 `vision` 链（agentrouter → NewAPI claude，支持图像输入）；新加 `tiny` 链（cline-free 日额满后 → codebuddy/glm-5.2 → deepseek）。修复 `slow` 链悬空引用 `zg-newapi/claude-opus-5`（不在 `models.yml` 中）。
